...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Line not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Line in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Shapes.Line {
<#
.Synopsis
   Create a new Line object
.Description
   Generates a new System.Windows.Shapes.Line object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Shapes.Line
   $All = Get-Parameter New-System.Windows.Shapes.Line | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Shapes.Line is not a collection
   # So if we're in a pipeline, make a new Line each time
   if($_) { 
      $DObject = New-Object System.Windows.Shapes.Line
   }
...
New-System.Windows.Shapes.Line @PSBoundParameters
