...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TableRow not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TableRow in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.TableRow {
<#
.Synopsis
   Create a new TableRow object
.Description
   Generates a new System.Windows.Documents.TableRow object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.TableRow
   $All = Get-Parameter New-System.Windows.Documents.TableRow | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.TableRow is not a collection
   # So if we're in a pipeline, make a new TableRow each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.TableRow
   }
...
New-System.Windows.Documents.TableRow @PSBoundParameters
