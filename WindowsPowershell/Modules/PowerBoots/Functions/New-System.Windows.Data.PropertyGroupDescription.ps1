...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "PropertyGroupDescription not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "PropertyGroupDescription in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Data.PropertyGroupDescription {
<#
.Synopsis
   Create a new PropertyGroupDescription object
.Description
   Generates a new System.Windows.Data.PropertyGroupDescription object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Data.PropertyGroupDescription
   $All = Get-Parameter New-System.Windows.Data.PropertyGroupDescription | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Data.PropertyGroupDescription is not a collection
   # So if we're in a pipeline, make a new PropertyGroupDescription each time
   if($_) { 
      $DObject = New-Object System.Windows.Data.PropertyGroupDescription
   }
...
New-System.Windows.Data.PropertyGroupDescription @PSBoundParameters
