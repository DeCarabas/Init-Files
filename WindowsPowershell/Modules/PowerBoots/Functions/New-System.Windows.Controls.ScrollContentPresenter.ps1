...
,
	[Parameter(ParameterSetName='Default')]
	[Switch]$CanContentScroll
,
	[Parameter(ParameterSetName='Default')]
	[Switch]$CanHorizontallyScroll
,
	[Parameter(ParameterSetName='Default')]
	[Switch]$CanVerticallyScroll
,
	[Parameter(ParameterSetName='Default')]
	[Object[]]$Clip
,
...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ScrollContentPresenter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ScrollContentPresenter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.ScrollContentPresenter {
<#
.Synopsis
   Create a new ScrollContentPresenter object
.Description
   Generates a new System.Windows.Controls.ScrollContentPresenter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.ScrollContentPresenter
   $All = Get-Parameter New-System.Windows.Controls.ScrollContentPresenter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.ScrollContentPresenter is not a collection
   # So if we're in a pipeline, make a new ScrollContentPresenter each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.ScrollContentPresenter
   }
...
New-System.Windows.Controls.ScrollContentPresenter @PSBoundParameters
