...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "CornerRadiusConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "CornerRadiusConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.CornerRadiusConverter {
<#
.Synopsis
   Create a new CornerRadiusConverter object
.Description
   Generates a new System.Windows.CornerRadiusConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.CornerRadiusConverter
   $All = Get-Parameter New-System.Windows.CornerRadiusConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.CornerRadiusConverter is not a collection
   # So if we're in a pipeline, make a new CornerRadiusConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.CornerRadiusConverter
   }
...
New-System.Windows.CornerRadiusConverter @PSBoundParameters
