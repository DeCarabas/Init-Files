...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ItemsControl not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ItemsControl in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.ItemsControl {
<#
.Synopsis
   Create a new ItemsControl object
.Description
   Generates a new System.Windows.Controls.ItemsControl object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.ItemsControl
   $All = Get-Parameter New-System.Windows.Controls.ItemsControl | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Controls.ItemsControl @PSBoundParameters
