...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "StyleTypedPropertyAttribute not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "StyleTypedPropertyAttribute in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.StyleTypedPropertyAttribute {
<#
.Synopsis
   Create a new StyleTypedPropertyAttribute object
.Description
   Generates a new System.Windows.StyleTypedPropertyAttribute object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.StyleTypedPropertyAttribute
   $All = Get-Parameter New-System.Windows.StyleTypedPropertyAttribute | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.StyleTypedPropertyAttribute is not a collection
   # So if we're in a pipeline, make a new StyleTypedPropertyAttribute each time
   if($_) { 
      $DObject = New-Object System.Windows.StyleTypedPropertyAttribute
   }
...
New-System.Windows.StyleTypedPropertyAttribute @PSBoundParameters
