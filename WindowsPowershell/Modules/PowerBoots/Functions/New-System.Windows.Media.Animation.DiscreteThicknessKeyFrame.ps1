...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "DiscreteThicknessKeyFrame not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "DiscreteThicknessKeyFrame in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Animation.DiscreteThicknessKeyFrame {
<#
.Synopsis
   Create a new DiscreteThicknessKeyFrame object
.Description
   Generates a new System.Windows.Media.Animation.DiscreteThicknessKeyFrame object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Animation.DiscreteThicknessKeyFrame
   $All = Get-Parameter New-System.Windows.Media.Animation.DiscreteThicknessKeyFrame | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Animation.DiscreteThicknessKeyFrame is not a collection
   # So if we're in a pipeline, make a new DiscreteThicknessKeyFrame each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Animation.DiscreteThicknessKeyFrame
   }
...
New-System.Windows.Media.Animation.DiscreteThicknessKeyFrame @PSBoundParameters
