...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ParagraphStructure not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ParagraphStructure in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentStructures.ParagraphStructure {
<#
.Synopsis
   Create a new ParagraphStructure object
.Description
   Generates a new System.Windows.Documents.DocumentStructures.ParagraphStructure object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentStructures.ParagraphStructure
   $All = Get-Parameter New-System.Windows.Documents.DocumentStructures.ParagraphStructure | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentStructures.ParagraphStructure is not a collection
   # So if we're in a pipeline, make a new ParagraphStructure each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentStructures.ParagraphStructure
   }
...
New-System.Windows.Documents.DocumentStructures.ParagraphStructure @PSBoundParameters
