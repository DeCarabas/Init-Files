...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "SkewTransform not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "SkewTransform in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.SkewTransform {
<#
.Synopsis
   Create a new SkewTransform object
.Description
   Generates a new System.Windows.Media.SkewTransform object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.SkewTransform
   $All = Get-Parameter New-System.Windows.Media.SkewTransform | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.SkewTransform is not a collection
   # So if we're in a pipeline, make a new SkewTransform each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.SkewTransform
   }
...
New-System.Windows.Media.SkewTransform @PSBoundParameters
