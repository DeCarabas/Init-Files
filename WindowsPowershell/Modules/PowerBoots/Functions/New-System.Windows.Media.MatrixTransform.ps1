...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "MatrixTransform not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "MatrixTransform in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.MatrixTransform {
<#
.Synopsis
   Create a new MatrixTransform object
.Description
   Generates a new System.Windows.Media.MatrixTransform object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.MatrixTransform
   $All = Get-Parameter New-System.Windows.Media.MatrixTransform | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.MatrixTransform is not a collection
   # So if we're in a pipeline, make a new MatrixTransform each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.MatrixTransform
   }
...
New-System.Windows.Media.MatrixTransform @PSBoundParameters
