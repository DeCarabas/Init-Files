...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "GridViewColumnHeader not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "GridViewColumnHeader in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.GridViewColumnHeader {
<#
.Synopsis
   Create a new GridViewColumnHeader object
.Description
   Generates a new System.Windows.Controls.GridViewColumnHeader object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.GridViewColumnHeader
   $All = Get-Parameter New-System.Windows.Controls.GridViewColumnHeader | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.GridViewColumnHeader is not a collection
   # So if we're in a pipeline, make a new GridViewColumnHeader each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.GridViewColumnHeader
   }
...
New-System.Windows.Controls.GridViewColumnHeader @PSBoundParameters
