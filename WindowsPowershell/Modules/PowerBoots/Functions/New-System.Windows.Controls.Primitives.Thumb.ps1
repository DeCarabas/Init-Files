...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Thumb not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Thumb in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Primitives.Thumb {
<#
.Synopsis
   Create a new Thumb object
.Description
   Generates a new System.Windows.Controls.Primitives.Thumb object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Primitives.Thumb
   $All = Get-Parameter New-System.Windows.Controls.Primitives.Thumb | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Primitives.Thumb is not a collection
   # So if we're in a pipeline, make a new Thumb each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Primitives.Thumb
   }
...
New-System.Windows.Controls.Primitives.Thumb @PSBoundParameters
