...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Hyperlink not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Hyperlink in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.Hyperlink {
<#
.Synopsis
   Create a new Hyperlink object
.Description
   Generates a new System.Windows.Documents.Hyperlink object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.Hyperlink
   $All = Get-Parameter New-System.Windows.Documents.Hyperlink | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Documents.Hyperlink @PSBoundParameters
