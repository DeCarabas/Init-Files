...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "BlockUIContainer not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "BlockUIContainer in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.BlockUIContainer {
<#
.Synopsis
   Create a new BlockUIContainer object
.Description
   Generates a new System.Windows.Documents.BlockUIContainer object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.BlockUIContainer
   $All = Get-Parameter New-System.Windows.Documents.BlockUIContainer | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.BlockUIContainer is not a collection
   # So if we're in a pipeline, make a new BlockUIContainer each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.BlockUIContainer
   }
...
New-System.Windows.Documents.BlockUIContainer @PSBoundParameters
