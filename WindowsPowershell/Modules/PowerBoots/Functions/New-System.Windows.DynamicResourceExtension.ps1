...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "DynamicResourceExtension not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "DynamicResourceExtension in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.DynamicResourceExtension {
<#
.Synopsis
   Create a new DynamicResourceExtension object
.Description
   Generates a new System.Windows.DynamicResourceExtension object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.DynamicResourceExtension
   $All = Get-Parameter New-System.Windows.DynamicResourceExtension | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.DynamicResourceExtension is not a collection
   # So if we're in a pipeline, make a new DynamicResourceExtension each time
   if($_) { 
      $DObject = New-Object System.Windows.DynamicResourceExtension
   }
...
New-System.Windows.DynamicResourceExtension @PSBoundParameters
