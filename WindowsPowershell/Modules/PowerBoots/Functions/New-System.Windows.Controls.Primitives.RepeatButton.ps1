...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "RepeatButton not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "RepeatButton in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Primitives.RepeatButton {
<#
.Synopsis
   Create a new RepeatButton object
.Description
   Generates a new System.Windows.Controls.Primitives.RepeatButton object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Primitives.RepeatButton
   $All = Get-Parameter New-System.Windows.Controls.Primitives.RepeatButton | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Primitives.RepeatButton is not a collection
   # So if we're in a pipeline, make a new RepeatButton each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Primitives.RepeatButton
   }
...
New-System.Windows.Controls.Primitives.RepeatButton @PSBoundParameters
