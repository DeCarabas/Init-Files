...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "RoutedEventConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "RoutedEventConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.RoutedEventConverter {
<#
.Synopsis
   Create a new RoutedEventConverter object
.Description
   Generates a new System.Windows.Markup.RoutedEventConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.RoutedEventConverter
   $All = Get-Parameter New-System.Windows.Markup.RoutedEventConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.RoutedEventConverter is not a collection
   # So if we're in a pipeline, make a new RoutedEventConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.RoutedEventConverter
   }
...
New-System.Windows.Markup.RoutedEventConverter @PSBoundParameters
