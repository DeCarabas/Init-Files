...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "StoryFragment not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "StoryFragment in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentStructures.StoryFragment {
<#
.Synopsis
   Create a new StoryFragment object
.Description
   Generates a new System.Windows.Documents.DocumentStructures.StoryFragment object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentStructures.StoryFragment
   $All = Get-Parameter New-System.Windows.Documents.DocumentStructures.StoryFragment | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentStructures.StoryFragment is not a collection
   # So if we're in a pipeline, make a new StoryFragment each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentStructures.StoryFragment
   }
...
New-System.Windows.Documents.DocumentStructures.StoryFragment @PSBoundParameters
