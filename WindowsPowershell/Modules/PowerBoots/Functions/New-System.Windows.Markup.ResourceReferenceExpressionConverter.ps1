...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ResourceReferenceExpressionConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ResourceReferenceExpressionConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.ResourceReferenceExpressionConverter {
<#
.Synopsis
   Create a new ResourceReferenceExpressionConverter object
.Description
   Generates a new System.Windows.Markup.ResourceReferenceExpressionConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.ResourceReferenceExpressionConverter
   $All = Get-Parameter New-System.Windows.Markup.ResourceReferenceExpressionConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.ResourceReferenceExpressionConverter is not a collection
   # So if we're in a pipeline, make a new ResourceReferenceExpressionConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.ResourceReferenceExpressionConverter
   }
...
New-System.Windows.Markup.ResourceReferenceExpressionConverter @PSBoundParameters
