...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ThicknessConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ThicknessConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.ThicknessConverter {
<#
.Synopsis
   Create a new ThicknessConverter object
.Description
   Generates a new System.Windows.ThicknessConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.ThicknessConverter
   $All = Get-Parameter New-System.Windows.ThicknessConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.ThicknessConverter is not a collection
   # So if we're in a pipeline, make a new ThicknessConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.ThicknessConverter
   }
...
New-System.Windows.ThicknessConverter @PSBoundParameters
