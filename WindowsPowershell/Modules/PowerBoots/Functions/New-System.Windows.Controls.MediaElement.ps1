...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "MediaElement not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "MediaElement in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.MediaElement {
<#
.Synopsis
   Create a new MediaElement object
.Description
   Generates a new System.Windows.Controls.MediaElement object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.MediaElement
   $All = Get-Parameter New-System.Windows.Controls.MediaElement | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.MediaElement is not a collection
   # So if we're in a pipeline, make a new MediaElement each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.MediaElement
   }
...
New-System.Windows.Controls.MediaElement @PSBoundParameters
