...
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_NavigationProgress
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_NavigationFailed
,
	[Parameter(ParameterSetName='Default')]
...
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_NavigationStopped
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_FragmentNavigation
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_SourceInitialized
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_Activated
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_Deactivated
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_StateChanged
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_LocationChanged
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_Closing
,
...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "NavigationWindow not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "NavigationWindow in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Navigation.NavigationWindow {
<#
.Synopsis
   Create a new NavigationWindow object
.Description
   Generates a new System.Windows.Navigation.NavigationWindow object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Navigation.NavigationWindow
   $All = Get-Parameter New-System.Windows.Navigation.NavigationWindow | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Navigation.NavigationWindow is not a collection
   # So if we're in a pipeline, make a new NavigationWindow each time
   if($_) { 
      $DObject = New-Object System.Windows.Navigation.NavigationWindow
   }
...
New-System.Windows.Navigation.NavigationWindow @PSBoundParameters
