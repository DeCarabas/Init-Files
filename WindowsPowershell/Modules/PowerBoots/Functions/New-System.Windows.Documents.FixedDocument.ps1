...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "FixedDocument not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "FixedDocument in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.FixedDocument {
<#
.Synopsis
   Create a new FixedDocument object
.Description
   Generates a new System.Windows.Documents.FixedDocument object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.FixedDocument
   $All = Get-Parameter New-System.Windows.Documents.FixedDocument | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.FixedDocument is not a collection
   # So if we're in a pipeline, make a new FixedDocument each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.FixedDocument
   }
...
New-System.Windows.Documents.FixedDocument @PSBoundParameters
