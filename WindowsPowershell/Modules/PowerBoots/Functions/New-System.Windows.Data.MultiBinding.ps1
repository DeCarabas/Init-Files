...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "MultiBinding not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "MultiBinding in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Data.MultiBinding {
<#
.Synopsis
   Create a new MultiBinding object
.Description
   Generates a new System.Windows.Data.MultiBinding object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Data.MultiBinding
   $All = Get-Parameter New-System.Windows.Data.MultiBinding | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Data.MultiBinding is not a collection
   # So if we're in a pipeline, make a new MultiBinding each time
   if($_) { 
      $DObject = New-Object System.Windows.Data.MultiBinding
   }
...
New-System.Windows.Data.MultiBinding @PSBoundParameters
