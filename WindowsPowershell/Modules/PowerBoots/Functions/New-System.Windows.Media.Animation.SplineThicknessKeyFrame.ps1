...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "SplineThicknessKeyFrame not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "SplineThicknessKeyFrame in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Animation.SplineThicknessKeyFrame {
<#
.Synopsis
   Create a new SplineThicknessKeyFrame object
.Description
   Generates a new System.Windows.Media.Animation.SplineThicknessKeyFrame object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Animation.SplineThicknessKeyFrame
   $All = Get-Parameter New-System.Windows.Media.Animation.SplineThicknessKeyFrame | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Animation.SplineThicknessKeyFrame is not a collection
   # So if we're in a pipeline, make a new SplineThicknessKeyFrame each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Animation.SplineThicknessKeyFrame
   }
...
New-System.Windows.Media.Animation.SplineThicknessKeyFrame @PSBoundParameters
