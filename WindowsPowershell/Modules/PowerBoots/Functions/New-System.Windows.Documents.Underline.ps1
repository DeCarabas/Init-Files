...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Underline not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Underline in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.Underline {
<#
.Synopsis
   Create a new Underline object
.Description
   Generates a new System.Windows.Documents.Underline object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.Underline
   $All = Get-Parameter New-System.Windows.Documents.Underline | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Documents.Underline @PSBoundParameters
