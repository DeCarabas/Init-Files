...
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_Unchecked
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_Indeterminate
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_Click
,
...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ToggleButton not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ToggleButton in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Primitives.ToggleButton {
<#
.Synopsis
   Create a new ToggleButton object
.Description
   Generates a new System.Windows.Controls.Primitives.ToggleButton object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Primitives.ToggleButton
   $All = Get-Parameter New-System.Windows.Controls.Primitives.ToggleButton | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Primitives.ToggleButton is not a collection
   # So if we're in a pipeline, make a new ToggleButton each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Primitives.ToggleButton
   }
...
New-System.Windows.Controls.Primitives.ToggleButton @PSBoundParameters
