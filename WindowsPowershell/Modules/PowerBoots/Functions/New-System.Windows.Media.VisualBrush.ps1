...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "VisualBrush not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "VisualBrush in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.VisualBrush {
<#
.Synopsis
   Create a new VisualBrush object
.Description
   Generates a new System.Windows.Media.VisualBrush object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.VisualBrush
   $All = Get-Parameter New-System.Windows.Media.VisualBrush | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.VisualBrush is not a collection
   # So if we're in a pipeline, make a new VisualBrush each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.VisualBrush
   }
...
New-System.Windows.Media.VisualBrush @PSBoundParameters
