...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "EventSetter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "EventSetter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.EventSetter {
<#
.Synopsis
   Create a new EventSetter object
.Description
   Generates a new System.Windows.EventSetter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.EventSetter
   $All = Get-Parameter New-System.Windows.EventSetter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.EventSetter is not a collection
   # So if we're in a pipeline, make a new EventSetter each time
   if($_) { 
      $DObject = New-Object System.Windows.EventSetter
   }
...
New-System.Windows.EventSetter @PSBoundParameters
