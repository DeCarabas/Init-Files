...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "GridLengthConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "GridLengthConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.GridLengthConverter {
<#
.Synopsis
   Create a new GridLengthConverter object
.Description
   Generates a new System.Windows.GridLengthConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.GridLengthConverter
   $All = Get-Parameter New-System.Windows.GridLengthConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.GridLengthConverter is not a collection
   # So if we're in a pipeline, make a new GridLengthConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.GridLengthConverter
   }
...
New-System.Windows.GridLengthConverter @PSBoundParameters
