...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "LineBreak not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "LineBreak in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.LineBreak {
<#
.Synopsis
   Create a new LineBreak object
.Description
   Generates a new System.Windows.Documents.LineBreak object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.LineBreak
   $All = Get-Parameter New-System.Windows.Documents.LineBreak | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.LineBreak is not a collection
   # So if we're in a pipeline, make a new LineBreak each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.LineBreak
   }
...
New-System.Windows.Documents.LineBreak @PSBoundParameters
