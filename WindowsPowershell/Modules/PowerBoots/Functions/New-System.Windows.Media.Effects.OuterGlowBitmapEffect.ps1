...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "OuterGlowBitmapEffect not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "OuterGlowBitmapEffect in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Effects.OuterGlowBitmapEffect {
<#
.Synopsis
   Create a new OuterGlowBitmapEffect object
.Description
   Generates a new System.Windows.Media.Effects.OuterGlowBitmapEffect object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Effects.OuterGlowBitmapEffect
   $All = Get-Parameter New-System.Windows.Media.Effects.OuterGlowBitmapEffect | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Effects.OuterGlowBitmapEffect is not a collection
   # So if we're in a pipeline, make a new OuterGlowBitmapEffect each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Effects.OuterGlowBitmapEffect
   }
...
New-System.Windows.Media.Effects.OuterGlowBitmapEffect @PSBoundParameters
