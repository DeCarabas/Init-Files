...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "InlineUIContainer not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "InlineUIContainer in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.InlineUIContainer {
<#
.Synopsis
   Create a new InlineUIContainer object
.Description
   Generates a new System.Windows.Documents.InlineUIContainer object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.InlineUIContainer
   $All = Get-Parameter New-System.Windows.Documents.InlineUIContainer | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.InlineUIContainer is not a collection
   # So if we're in a pipeline, make a new InlineUIContainer each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.InlineUIContainer
   }
...
New-System.Windows.Documents.InlineUIContainer @PSBoundParameters
