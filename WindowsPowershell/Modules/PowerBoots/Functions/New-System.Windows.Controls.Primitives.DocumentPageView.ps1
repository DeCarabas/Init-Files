...
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_PageConnected
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_PageDisconnected
,
	[Parameter(ParameterSetName='Default')]
...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "DocumentPageView not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "DocumentPageView in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Primitives.DocumentPageView {
<#
.Synopsis
   Create a new DocumentPageView object
.Description
   Generates a new System.Windows.Controls.Primitives.DocumentPageView object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Primitives.DocumentPageView
   $All = Get-Parameter New-System.Windows.Controls.Primitives.DocumentPageView | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Primitives.DocumentPageView is not a collection
   # So if we're in a pipeline, make a new DocumentPageView each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Primitives.DocumentPageView
   }
...
New-System.Windows.Controls.Primitives.DocumentPageView @PSBoundParameters
