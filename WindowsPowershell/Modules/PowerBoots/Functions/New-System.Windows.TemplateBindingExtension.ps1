...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TemplateBindingExtension not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TemplateBindingExtension in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.TemplateBindingExtension {
<#
.Synopsis
   Create a new TemplateBindingExtension object
.Description
   Generates a new System.Windows.TemplateBindingExtension object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.TemplateBindingExtension
   $All = Get-Parameter New-System.Windows.TemplateBindingExtension | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.TemplateBindingExtension is not a collection
   # So if we're in a pipeline, make a new TemplateBindingExtension each time
   if($_) { 
      $DObject = New-Object System.Windows.TemplateBindingExtension
   }
...
New-System.Windows.TemplateBindingExtension @PSBoundParameters
