...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TabItem not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TabItem in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.TabItem {
<#
.Synopsis
   Create a new TabItem object
.Description
   Generates a new System.Windows.Controls.TabItem object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.TabItem
   $All = Get-Parameter New-System.Windows.Controls.TabItem | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.TabItem is not a collection
   # So if we're in a pipeline, make a new TabItem each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.TabItem
   }
...
New-System.Windows.Controls.TabItem @PSBoundParameters
