...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "BeginStoryboard not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "BeginStoryboard in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Animation.BeginStoryboard {
<#
.Synopsis
   Create a new BeginStoryboard object
.Description
   Generates a new System.Windows.Media.Animation.BeginStoryboard object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Animation.BeginStoryboard
   $All = Get-Parameter New-System.Windows.Media.Animation.BeginStoryboard | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Animation.BeginStoryboard is not a collection
   # So if we're in a pipeline, make a new BeginStoryboard each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Animation.BeginStoryboard
   }
...
New-System.Windows.Media.Animation.BeginStoryboard @PSBoundParameters
