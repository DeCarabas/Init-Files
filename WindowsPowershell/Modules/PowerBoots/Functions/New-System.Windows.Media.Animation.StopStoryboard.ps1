...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "StopStoryboard not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "StopStoryboard in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Animation.StopStoryboard {
<#
.Synopsis
   Create a new StopStoryboard object
.Description
   Generates a new System.Windows.Media.Animation.StopStoryboard object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Animation.StopStoryboard
   $All = Get-Parameter New-System.Windows.Media.Animation.StopStoryboard | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Animation.StopStoryboard is not a collection
   # So if we're in a pipeline, make a new StopStoryboard each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Animation.StopStoryboard
   }
...
New-System.Windows.Media.Animation.StopStoryboard @PSBoundParameters
