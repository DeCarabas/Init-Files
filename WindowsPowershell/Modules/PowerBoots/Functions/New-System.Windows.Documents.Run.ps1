...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Run not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Run in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.Run {
<#
.Synopsis
   Create a new Run object
.Description
   Generates a new System.Windows.Documents.Run object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.Run
   $All = Get-Parameter New-System.Windows.Documents.Run | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.Run is not a collection
   # So if we're in a pipeline, make a new Run each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.Run
   }
...
New-System.Windows.Documents.Run @PSBoundParameters
