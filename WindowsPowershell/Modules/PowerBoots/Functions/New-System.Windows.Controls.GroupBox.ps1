...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "GroupBox not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "GroupBox in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.GroupBox {
<#
.Synopsis
   Create a new GroupBox object
.Description
   Generates a new System.Windows.Controls.GroupBox object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.GroupBox
   $All = Get-Parameter New-System.Windows.Controls.GroupBox | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.GroupBox is not a collection
   # So if we're in a pipeline, make a new GroupBox each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.GroupBox
   }
...
New-System.Windows.Controls.GroupBox @PSBoundParameters
