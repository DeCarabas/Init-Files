...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Window not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Window in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Window {
<#
.Synopsis
   Create a new Window object
.Description
   Generates a new System.Windows.Window object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Window
   $All = Get-Parameter New-System.Windows.Window | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Window is not a collection
   # So if we're in a pipeline, make a new Window each time
   if($_) { 
      $DObject = New-Object System.Windows.Window
   }
...
New-System.Windows.Window @PSBoundParameters
