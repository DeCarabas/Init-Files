...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Bold not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Bold in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.Bold {
<#
.Synopsis
   Create a new Bold object
.Description
   Generates a new System.Windows.Documents.Bold object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.Bold
   $All = Get-Parameter New-System.Windows.Documents.Bold | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Documents.Bold @PSBoundParameters
