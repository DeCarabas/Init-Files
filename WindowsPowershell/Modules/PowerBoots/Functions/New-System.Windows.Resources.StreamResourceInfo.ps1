...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "StreamResourceInfo not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "StreamResourceInfo in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Resources.StreamResourceInfo {
<#
.Synopsis
   Create a new StreamResourceInfo object
.Description
   Generates a new System.Windows.Resources.StreamResourceInfo object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Resources.StreamResourceInfo
   $All = Get-Parameter New-System.Windows.Resources.StreamResourceInfo | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Resources.StreamResourceInfo is not a collection
   # So if we're in a pipeline, make a new StreamResourceInfo each time
   if($_) { 
      $DObject = New-Object System.Windows.Resources.StreamResourceInfo
   }
...
New-System.Windows.Resources.StreamResourceInfo @PSBoundParameters
