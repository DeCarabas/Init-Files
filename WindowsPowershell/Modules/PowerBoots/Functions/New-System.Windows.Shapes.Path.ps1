...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Path not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Path in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Shapes.Path {
<#
.Synopsis
   Create a new Path object
.Description
   Generates a new System.Windows.Shapes.Path object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Shapes.Path
   $All = Get-Parameter New-System.Windows.Shapes.Path | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Shapes.Path is not a collection
   # So if we're in a pipeline, make a new Path each time
   if($_) { 
      $DObject = New-Object System.Windows.Shapes.Path
   }
...
New-System.Windows.Shapes.Path @PSBoundParameters
