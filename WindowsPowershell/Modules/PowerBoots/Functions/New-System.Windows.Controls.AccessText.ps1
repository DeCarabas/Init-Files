...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "AccessText not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "AccessText in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.AccessText {
<#
.Synopsis
   Create a new AccessText object
.Description
   Generates a new System.Windows.Controls.AccessText object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.AccessText
   $All = Get-Parameter New-System.Windows.Controls.AccessText | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.AccessText is not a collection
   # So if we're in a pipeline, make a new AccessText each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.AccessText
   }
...
New-System.Windows.Controls.AccessText @PSBoundParameters
