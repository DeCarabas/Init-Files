...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "XamlParseException not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "XamlParseException in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.XamlParseException {
<#
.Synopsis
   Create a new XamlParseException object
.Description
   Generates a new System.Windows.Markup.XamlParseException object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.XamlParseException
   $All = Get-Parameter New-System.Windows.Markup.XamlParseException | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.XamlParseException is not a collection
   # So if we're in a pipeline, make a new XamlParseException each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.XamlParseException
   }
...
New-System.Windows.Markup.XamlParseException @PSBoundParameters
