...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "CollectionViewSource not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "CollectionViewSource in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Data.CollectionViewSource {
<#
.Synopsis
   Create a new CollectionViewSource object
.Description
   Generates a new System.Windows.Data.CollectionViewSource object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Data.CollectionViewSource
   $All = Get-Parameter New-System.Windows.Data.CollectionViewSource | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Data.CollectionViewSource is not a collection
   # So if we're in a pipeline, make a new CollectionViewSource each time
   if($_) { 
      $DObject = New-Object System.Windows.Data.CollectionViewSource
   }
...
New-System.Windows.Data.CollectionViewSource @PSBoundParameters
