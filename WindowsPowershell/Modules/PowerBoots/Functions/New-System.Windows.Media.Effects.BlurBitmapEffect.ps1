...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "BlurBitmapEffect not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "BlurBitmapEffect in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Effects.BlurBitmapEffect {
<#
.Synopsis
   Create a new BlurBitmapEffect object
.Description
   Generates a new System.Windows.Media.Effects.BlurBitmapEffect object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Effects.BlurBitmapEffect
   $All = Get-Parameter New-System.Windows.Media.Effects.BlurBitmapEffect | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Effects.BlurBitmapEffect is not a collection
   # So if we're in a pipeline, make a new BlurBitmapEffect each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Effects.BlurBitmapEffect
   }
...
New-System.Windows.Media.Effects.BlurBitmapEffect @PSBoundParameters
