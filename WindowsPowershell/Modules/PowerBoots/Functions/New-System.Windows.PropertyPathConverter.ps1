...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "PropertyPathConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "PropertyPathConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.PropertyPathConverter {
<#
.Synopsis
   Create a new PropertyPathConverter object
.Description
   Generates a new System.Windows.PropertyPathConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.PropertyPathConverter
   $All = Get-Parameter New-System.Windows.PropertyPathConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.PropertyPathConverter is not a collection
   # So if we're in a pipeline, make a new PropertyPathConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.PropertyPathConverter
   }
...
New-System.Windows.PropertyPathConverter @PSBoundParameters
