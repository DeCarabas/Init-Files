...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TableRowStructure not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TableRowStructure in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentStructures.TableRowStructure {
<#
.Synopsis
   Create a new TableRowStructure object
.Description
   Generates a new System.Windows.Documents.DocumentStructures.TableRowStructure object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentStructures.TableRowStructure
   $All = Get-Parameter New-System.Windows.Documents.DocumentStructures.TableRowStructure | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentStructures.TableRowStructure is not a collection
   # So if we're in a pipeline, make a new TableRowStructure each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentStructures.TableRowStructure
   }
...
New-System.Windows.Documents.DocumentStructures.TableRowStructure @PSBoundParameters
