...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "MultiDataTrigger not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "MultiDataTrigger in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.MultiDataTrigger {
<#
.Synopsis
   Create a new MultiDataTrigger object
.Description
   Generates a new System.Windows.MultiDataTrigger object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.MultiDataTrigger
   $All = Get-Parameter New-System.Windows.MultiDataTrigger | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.MultiDataTrigger is not a collection
   # So if we're in a pipeline, make a new MultiDataTrigger each time
   if($_) { 
      $DObject = New-Object System.Windows.MultiDataTrigger
   }
...
New-System.Windows.MultiDataTrigger @PSBoundParameters
