...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "StaticExtension not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "StaticExtension in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.StaticExtension {
<#
.Synopsis
   Create a new StaticExtension object
.Description
   Generates a new System.Windows.Markup.StaticExtension object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.StaticExtension
   $All = Get-Parameter New-System.Windows.Markup.StaticExtension | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.StaticExtension is not a collection
   # So if we're in a pipeline, make a new StaticExtension each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.StaticExtension
   }
...
New-System.Windows.Markup.StaticExtension @PSBoundParameters
