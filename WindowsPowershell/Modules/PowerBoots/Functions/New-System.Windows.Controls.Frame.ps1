...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Frame not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Frame in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Frame {
<#
.Synopsis
   Create a new Frame object
.Description
   Generates a new System.Windows.Controls.Frame object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Frame
   $All = Get-Parameter New-System.Windows.Controls.Frame | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Frame is not a collection
   # So if we're in a pipeline, make a new Frame each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Frame
   }
...
New-System.Windows.Controls.Frame @PSBoundParameters
