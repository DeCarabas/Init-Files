...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Table not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Table in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.Table {
<#
.Synopsis
   Create a new Table object
.Description
   Generates a new System.Windows.Documents.Table object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.Table
   $All = Get-Parameter New-System.Windows.Documents.Table | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.Table is not a collection
   # So if we're in a pipeline, make a new Table each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.Table
   }
...
New-System.Windows.Documents.Table @PSBoundParameters
