...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "AdornerDecorator not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "AdornerDecorator in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.AdornerDecorator {
<#
.Synopsis
   Create a new AdornerDecorator object
.Description
   Generates a new System.Windows.Documents.AdornerDecorator object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.AdornerDecorator
   $All = Get-Parameter New-System.Windows.Documents.AdornerDecorator | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.AdornerDecorator is not a collection
   # So if we're in a pipeline, make a new AdornerDecorator each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.AdornerDecorator
   }
...
New-System.Windows.Documents.AdornerDecorator @PSBoundParameters
