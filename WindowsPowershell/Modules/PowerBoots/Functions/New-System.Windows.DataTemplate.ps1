...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "DataTemplate not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "DataTemplate in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.DataTemplate {
<#
.Synopsis
   Create a new DataTemplate object
.Description
   Generates a new System.Windows.DataTemplate object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.DataTemplate
   $All = Get-Parameter New-System.Windows.DataTemplate | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.DataTemplate is not a collection
   # So if we're in a pipeline, make a new DataTemplate each time
   if($_) { 
      $DObject = New-Object System.Windows.DataTemplate
   }
...
New-System.Windows.DataTemplate @PSBoundParameters
