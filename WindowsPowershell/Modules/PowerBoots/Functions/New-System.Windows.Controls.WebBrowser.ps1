...
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_Navigating
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_Navigated
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_LoadCompleted
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_MessageHook
,
	[Parameter(ParameterSetName='Default')]
...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "WebBrowser not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "WebBrowser in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.WebBrowser {
<#
.Synopsis
   Create a new WebBrowser object
.Description
   Generates a new System.Windows.Controls.WebBrowser object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.WebBrowser
   $All = Get-Parameter New-System.Windows.Controls.WebBrowser | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.WebBrowser is not a collection
   # So if we're in a pipeline, make a new WebBrowser each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.WebBrowser
   }
...
New-System.Windows.Controls.WebBrowser @PSBoundParameters
