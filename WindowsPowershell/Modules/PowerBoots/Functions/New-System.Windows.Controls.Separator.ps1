...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Separator not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Separator in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Separator {
<#
.Synopsis
   Create a new Separator object
.Description
   Generates a new System.Windows.Controls.Separator object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Separator
   $All = Get-Parameter New-System.Windows.Controls.Separator | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Separator is not a collection
   # So if we're in a pipeline, make a new Separator each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Separator
   }
...
New-System.Windows.Controls.Separator @PSBoundParameters
