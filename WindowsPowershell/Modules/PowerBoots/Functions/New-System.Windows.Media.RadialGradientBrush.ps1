...
,
	[Parameter(ParameterSetName='Default')]
	[Object[]]$ColorInterpolationMode
,
	[Parameter(ParameterSetName='Default')]
	[Object[]]$GradientOrigin
,
...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "RadialGradientBrush not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "RadialGradientBrush in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.RadialGradientBrush {
<#
.Synopsis
   Create a new RadialGradientBrush object
.Description
   Generates a new System.Windows.Media.RadialGradientBrush object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.RadialGradientBrush
   $All = Get-Parameter New-System.Windows.Media.RadialGradientBrush | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Media.RadialGradientBrush @PSBoundParameters
