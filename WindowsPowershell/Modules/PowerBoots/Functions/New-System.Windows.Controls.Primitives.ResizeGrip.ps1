...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ResizeGrip not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ResizeGrip in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Primitives.ResizeGrip {
<#
.Synopsis
   Create a new ResizeGrip object
.Description
   Generates a new System.Windows.Controls.Primitives.ResizeGrip object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Primitives.ResizeGrip
   $All = Get-Parameter New-System.Windows.Controls.Primitives.ResizeGrip | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Primitives.ResizeGrip is not a collection
   # So if we're in a pipeline, make a new ResizeGrip each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Primitives.ResizeGrip
   }
...
New-System.Windows.Controls.Primitives.ResizeGrip @PSBoundParameters
