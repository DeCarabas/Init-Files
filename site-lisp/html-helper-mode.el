...
;; Created: 01 Feb 1994
;; $Id: html-helper-mode.el,v 3.0.3.3 1998/08/06 18:53:03 nelson Exp $
...
;; Added a defcustom for a symbol meant to tell Emacs that it has to consider
;; asp/php/jsp blocks beginning in column 1

;; Added messages in the minibuffer while matching PHP/JSP/PHP blocks

;; Added support for JSP (ALPHA)

;; Added a defcustom for a symbol meant to tell Emacs that the page contains
;; JSP and not ASP
...
(defconst html-helper-mode-version (substring "$Revision: 3.0.3.3 $" 11 15))
...
(defcustom html-helper-mode-uses-csharp t
  "Non nil to require csharp-mode"
  :type 'boolean
  :initialize 'custom-initialize-default
  :group 'html-helper
  :require 'html-helper-mode)
...
(defvar html-helper-new-ASPX-buffer-template
  '("<%@ LANGUAGE=\"" p "\" %>\n"
    "<html> <head>\n"
    "<%\n\n%>\n"
    "</head><body>\n"
    ...
    " --> %>\n"
    "\n</body></html>\n")
"*Template for new ASPX buffers.
Inserted by `html-helper-insert-new-ASPX-buffer-strings' if
`html-helper-build-new-buffer' is set to t")
...
;; inserts c# script tag
(define-key html-helper-mode-functions-map "s"
  'html-helper-insert-csharp-script-tag)
...
This function can be called again, it redoes the entire menu."
  ...
  ;; cons in the C# script block insert
  (setq html-helper-mode-menu
	(cons '["Insert CSharp Script Block"
		html-helper-insert-csharp-script-tag t]
	      html-helper-mode-menu))
  ...
  (setq html-helper-mode-menu
	(append html-helper-mode-menu 
		(list ["Narrow to CSharp" html-script-narrow-to-csharp t])))
...
(defun html-helper-insert-csharp-script-tag ()
  ...
  (interactive)
  (insert "<script language=\"C#\" runat=\"server\">\n\n</script>")
  (search-backward "\n" nil t)
)
;;}}}
...
is the mode to run (that's the skeleton to insert in empty files)"
  ...
  (if (and html-helper-build-new-buffer (zerop (buffer-size)))
      (cond ((string= mode "HTML")
	    ...
	    ((string= mode "ASPX")
	     (html-helper-insert-new-ASPX-buffer-strings))
	    ((string= mode "PHP")
...
(cond (html-helper-mode-uses-csharp
       (defun aspx-html-helper-mode ()
  "Mode for editing HTML documents with ASPX server scripts.
...
Uses :`cc-mode'  for ASP editing of CSharp
      `easymenu' for menu creation
      `cc-mode'  for javascript support
      `tempo'    for templates
Supports server (actually ASP, ASPX, PHP, JSP) and client
(JavaScript, VBScript) scripting

Customizable flags you would like to alter

`html-helper-mode-uses-csharp' : non nil requires cc-mode and activates ASP and CSharp support functions
`html-helper-mode-uses-bold-italic' : non nil creates a bold italic face (could fail if there's not such face available)
`html-helper-mode-uses-KG-style' : nil to make Emacs consider PHP/JSP/ASP/ASPX code blocks beginning in the first colum
`html-helper-mode-global-JSP-not-ASP' : non nil to make Emacs consider <% %> sequence as JSP blocks by default in html-helper-mode, set to nil in asp-html-helper-mode, set to t in jsp-html-helper-mode. 
Alter the behaviour locally by changing html-helper-mode-local-JSP-not-ASP value
...
Special command (not in menu): [f4] attempts a smart narrowing to the current scripting block. Fails in client script containing server script sections.
\\{html-helper-mode-map}
Written by bmerrill@microsoft.com
"
  (interactive)
  (base-html-helper-mode "ASPX")
  (setq html-helper-mode-local-JSP-not-ASP nil)
  (setq mode-name "HTML/ASPX helper")
  (setq major-mode 'aspx-html-helper-mode)
)))
...
Uses :`visual-basic-mode' (optional - see below ) for ASP and VBScript
      `easymenu' for menu creation 
      `cc-mode'  for javascript/csharp support
      `tempo'    for templates
...
`html-helper-mode-uses-visual-basic' : non nil requires visual-basic-mode and activates ASP and VBScript support functions
`html-helper-mode-uses-csharp' : non nil requires cc-mode and activates ASPX and CSharp support functions
`html-helper-mode-uses-bold-italic' : non nil creates a bold italic face (could fail if there's not such face available)
`html-helper-mode-uses-KG-style' : nil to make Emacs consider PHP/JSP/ASP code blocks beginning in the first colum
`html-helper-mode-global-JSP-not-ASP' : non nil to make Emacs consider <% %> sequence as JSP blocks by default in html-helper-mode, set to nil in asp-html-helper-mode, set to t in jsp-html-helper-mode. 
Alter the behaviour locally by changing html-helper-mode-local-JSP-not-ASP value
...
(defun jsp-html-helper-mode ()
  "Mode for editing HTML documents with JSP server scripts.
...
(defadvice html-script-narrow-to-vbscript (around save-excursion activate)
  (save-excursion
    ad-do-it))
(defadvice html-script-narrow-to-csharp (around save-excursion activate)
  (save-excursion
    ad-do-it))
...
(defun html-script-narrow-to-asp ()
  ...
  (interactive)
  (cond ((or html-helper-mode-uses-visual-basic
...
(defun html-script-narrow-to-aspx ()
  "Narrows to an ASP script and setups csharp mode. Does nothing if html-helper-mode-uses-csharp is nil"
  (interactive)
  (cond ((or html-helper-mode-uses-csharp
	     html-helper-mode-local-JSP-not-ASP)
	 (cond ( html-helper-mode-uses-KG-style
		 (search-backward-regexp "^<%") )
	       (t (search-backward "<%" )))
...
(defun html-script-narrow-to-vbscript ()
  ...
  (cond (html-helper-mode-uses-visual-basic
	 (search-backward-regexp
	  "<\\(SCRIPT|script\\)[ \t]+\\(\\(LANGUAGE|language\\)=\"VBScript\"\\|TYPE=\"text/vbscript\"\\)[ \t]*>")
	 (let ((beg (point)))
	   (search-forward "</\\(SCRIPT|script\\)>" nil t)
	   (narrow-to-region beg (point)))
	 (visual-basic-mode)
	 (goto-char 0))))

(defun html-script-narrow-to-csharp ()
  "Narrows to CSharp and sets up csharp mode. Does nothing if html-helper-mode-uses-csharp is nil"
  (interactive)
    (cond (html-helper-mode-uses-csharp
;;	   (search-backward-regexp "<\\(SCRIPT|script\\)[ \t]+\\(\\(LANGUAGE|language\\)=\"[Cc]#\"\\|TYPE=\"text/c#\"\\)[ \t]*>")
	(search-backward-regexp "<script\\s +\\(\\(language=\"c#\"\\)\\|\\(type=\"text/c#\"\\)\\)\\s +")
;;	(search-backward-regexp "<script\\s +language=\"c#\"\\s +")
	(search-forward "\n" nil t)
	(let ((beg (point)))
...
(defun html-script-narrow-to-javascript ()
  ...
  (interactive)
    (search-backward-regexp "<\\(SCRIPT|script\\)[ \t]+\\(\\(LANGUAGE|language\\)=\"JavaScript\"\\|TYPE=\"text/javascript\"\\)[ \t]*>")
    (let ((beg (point)))
      (search-forward "</\\(SCRIPT|script\\)>" nil t)
      (narrow-to-region beg (point)))
  (java-mode)
...
(cond 
 (csharp-mode-hook
  (add-hook 'csharp-mode-hook 'html-script-release-setup))
...
(defvar html-script-narrow-alist
  `((,(regexp-quote "<%") . html-script-narrow-to-asp)
    ...
    ("<script\\s +language=\"vbscript\"\\s +" . html-script-narrow-to-vbscript)
    ("<script\\s +type=\"text/vbscript\"\\s +" . html-script-narrow-to-vbscript)
    ("<script\\s +language=\"javascript\"\\s +" .
     html-script-narrow-to-javascript)
    ("<script\\s +type=\"text/javascript\"\\s +>" .
     html-script-narrow-to-javascript)
    ("<script\\s +language=\"c#\"\\s +" . html-script-narrow-to-csharp)
    ("<script\\s +type=\"text/c#\"\\s +" . html-script-narrow-to-csharp)
    ))
