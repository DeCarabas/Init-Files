...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TableCellStructure not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TableCellStructure in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentStructures.TableCellStructure {
<#
.Synopsis
   Create a new TableCellStructure object
.Description
   Generates a new System.Windows.Documents.DocumentStructures.TableCellStructure object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentStructures.TableCellStructure
   $All = Get-Parameter New-System.Windows.Documents.DocumentStructures.TableCellStructure | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentStructures.TableCellStructure is not a collection
   # So if we're in a pipeline, make a new TableCellStructure each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentStructures.TableCellStructure
   }
...
New-System.Windows.Documents.DocumentStructures.TableCellStructure @PSBoundParameters
