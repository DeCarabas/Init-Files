...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ValueUnavailableException not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ValueUnavailableException in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Data.ValueUnavailableException {
<#
.Synopsis
   Create a new ValueUnavailableException object
.Description
   Generates a new System.Windows.Data.ValueUnavailableException object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Data.ValueUnavailableException
   $All = Get-Parameter New-System.Windows.Data.ValueUnavailableException | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Data.ValueUnavailableException is not a collection
   # So if we're in a pipeline, make a new ValueUnavailableException each time
   if($_) { 
      $DObject = New-Object System.Windows.Data.ValueUnavailableException
   }
...
New-System.Windows.Data.ValueUnavailableException @PSBoundParameters
