...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Expander not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Expander in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Expander {
<#
.Synopsis
   Create a new Expander object
.Description
   Generates a new System.Windows.Controls.Expander object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Expander
   $All = Get-Parameter New-System.Windows.Controls.Expander | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Expander is not a collection
   # So if we're in a pipeline, make a new Expander each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Expander
   }
...
New-System.Windows.Controls.Expander @PSBoundParameters
