...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "StoryBreak not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "StoryBreak in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentStructures.StoryBreak {
<#
.Synopsis
   Create a new StoryBreak object
.Description
   Generates a new System.Windows.Documents.DocumentStructures.StoryBreak object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentStructures.StoryBreak
   $All = Get-Parameter New-System.Windows.Documents.DocumentStructures.StoryBreak | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentStructures.StoryBreak is not a collection
   # So if we're in a pipeline, make a new StoryBreak each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentStructures.StoryBreak
   }
...
New-System.Windows.Documents.DocumentStructures.StoryBreak @PSBoundParameters
