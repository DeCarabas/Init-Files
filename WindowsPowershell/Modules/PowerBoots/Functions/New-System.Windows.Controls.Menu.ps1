...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Menu not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Menu in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Menu {
<#
.Synopsis
   Create a new Menu object
.Description
   Generates a new System.Windows.Controls.Menu object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Menu
   $All = Get-Parameter New-System.Windows.Controls.Menu | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Controls.Menu @PSBoundParameters
