...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ThicknessKeyFrameCollection not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ThicknessKeyFrameCollection in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Animation.ThicknessKeyFrameCollection {
<#
.Synopsis
   Create a new ThicknessKeyFrameCollection object
.Description
   Generates a new System.Windows.Media.Animation.ThicknessKeyFrameCollection object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Animation.ThicknessKeyFrameCollection
   $All = Get-Parameter New-System.Windows.Media.Animation.ThicknessKeyFrameCollection | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Animation.ThicknessKeyFrameCollection is not a collection
   # So if we're in a pipeline, make a new ThicknessKeyFrameCollection each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Animation.ThicknessKeyFrameCollection
   }
...
New-System.Windows.Media.Animation.ThicknessKeyFrameCollection @PSBoundParameters
