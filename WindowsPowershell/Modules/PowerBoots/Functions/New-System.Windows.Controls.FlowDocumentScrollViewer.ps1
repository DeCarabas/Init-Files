...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "FlowDocumentScrollViewer not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "FlowDocumentScrollViewer in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.FlowDocumentScrollViewer {
<#
.Synopsis
   Create a new FlowDocumentScrollViewer object
.Description
   Generates a new System.Windows.Controls.FlowDocumentScrollViewer object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.FlowDocumentScrollViewer
   $All = Get-Parameter New-System.Windows.Controls.FlowDocumentScrollViewer | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.FlowDocumentScrollViewer is not a collection
   # So if we're in a pipeline, make a new FlowDocumentScrollViewer each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.FlowDocumentScrollViewer
   }
...
New-System.Windows.Controls.FlowDocumentScrollViewer @PSBoundParameters
