...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Floater not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Floater in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.Floater {
<#
.Synopsis
   Create a new Floater object
.Description
   Generates a new System.Windows.Documents.Floater object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.Floater
   $All = Get-Parameter New-System.Windows.Documents.Floater | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Documents.Floater @PSBoundParameters
