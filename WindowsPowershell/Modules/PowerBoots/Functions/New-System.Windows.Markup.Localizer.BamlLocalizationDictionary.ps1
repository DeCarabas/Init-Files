...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "BamlLocalizationDictionary not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "BamlLocalizationDictionary in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.Localizer.BamlLocalizationDictionary {
<#
.Synopsis
   Create a new BamlLocalizationDictionary object
.Description
   Generates a new System.Windows.Markup.Localizer.BamlLocalizationDictionary object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.Localizer.BamlLocalizationDictionary
   $All = Get-Parameter New-System.Windows.Markup.Localizer.BamlLocalizationDictionary | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.Localizer.BamlLocalizationDictionary is not a collection
   # So if we're in a pipeline, make a new BamlLocalizationDictionary each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.Localizer.BamlLocalizationDictionary
   }
...
New-System.Windows.Markup.Localizer.BamlLocalizationDictionary @PSBoundParameters
