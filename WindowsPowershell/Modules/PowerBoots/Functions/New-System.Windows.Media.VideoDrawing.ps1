...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "VideoDrawing not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "VideoDrawing in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.VideoDrawing {
<#
.Synopsis
   Create a new VideoDrawing object
.Description
   Generates a new System.Windows.Media.VideoDrawing object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.VideoDrawing
   $All = Get-Parameter New-System.Windows.Media.VideoDrawing | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.VideoDrawing is not a collection
   # So if we're in a pipeline, make a new VideoDrawing each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.VideoDrawing
   }
...
New-System.Windows.Media.VideoDrawing @PSBoundParameters
