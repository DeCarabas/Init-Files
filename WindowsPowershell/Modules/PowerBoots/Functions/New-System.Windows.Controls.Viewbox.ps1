...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Viewbox not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Viewbox in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Viewbox {
<#
.Synopsis
   Create a new Viewbox object
.Description
   Generates a new System.Windows.Controls.Viewbox object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Viewbox
   $All = Get-Parameter New-System.Windows.Controls.Viewbox | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Viewbox is not a collection
   # So if we're in a pipeline, make a new Viewbox each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Viewbox
   }
...
New-System.Windows.Controls.Viewbox @PSBoundParameters
