...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ResumeStoryboard not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ResumeStoryboard in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Animation.ResumeStoryboard {
<#
.Synopsis
   Create a new ResumeStoryboard object
.Description
   Generates a new System.Windows.Media.Animation.ResumeStoryboard object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Animation.ResumeStoryboard
   $All = Get-Parameter New-System.Windows.Media.Animation.ResumeStoryboard | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Animation.ResumeStoryboard is not a collection
   # So if we're in a pipeline, make a new ResumeStoryboard each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Animation.ResumeStoryboard
   }
...
New-System.Windows.Media.Animation.ResumeStoryboard @PSBoundParameters
