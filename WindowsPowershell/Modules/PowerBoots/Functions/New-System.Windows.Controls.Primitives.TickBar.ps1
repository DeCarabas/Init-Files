...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TickBar not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TickBar in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Primitives.TickBar {
<#
.Synopsis
   Create a new TickBar object
.Description
   Generates a new System.Windows.Controls.Primitives.TickBar object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Primitives.TickBar
   $All = Get-Parameter New-System.Windows.Controls.Primitives.TickBar | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Primitives.TickBar is not a collection
   # So if we're in a pipeline, make a new TickBar each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Primitives.TickBar
   }
...
New-System.Windows.Controls.Primitives.TickBar @PSBoundParameters
