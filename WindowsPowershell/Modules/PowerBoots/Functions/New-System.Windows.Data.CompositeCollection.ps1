...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "CompositeCollection not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "CompositeCollection in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Data.CompositeCollection {
<#
.Synopsis
   Create a new CompositeCollection object
.Description
   Generates a new System.Windows.Data.CompositeCollection object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Data.CompositeCollection
   $All = Get-Parameter New-System.Windows.Data.CompositeCollection | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Data.CompositeCollection is not a collection
   # So if we're in a pipeline, make a new CompositeCollection each time
   if($_) { 
      $DObject = New-Object System.Windows.Data.CompositeCollection
   }
...
New-System.Windows.Data.CompositeCollection @PSBoundParameters
