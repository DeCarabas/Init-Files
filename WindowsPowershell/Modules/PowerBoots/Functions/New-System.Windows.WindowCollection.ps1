...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "WindowCollection not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "WindowCollection in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.WindowCollection {
<#
.Synopsis
   Create a new WindowCollection object
.Description
   Generates a new System.Windows.WindowCollection object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.WindowCollection
   $All = Get-Parameter New-System.Windows.WindowCollection | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.WindowCollection is not a collection
   # So if we're in a pipeline, make a new WindowCollection each time
   if($_) { 
      $DObject = New-Object System.Windows.WindowCollection
   }
...
New-System.Windows.WindowCollection @PSBoundParameters
