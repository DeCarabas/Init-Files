...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Track not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Track in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Primitives.Track {
<#
.Synopsis
   Create a new Track object
.Description
   Generates a new System.Windows.Controls.Primitives.Track object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Primitives.Track
   $All = Get-Parameter New-System.Windows.Controls.Primitives.Track | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Primitives.Track is not a collection
   # So if we're in a pipeline, make a new Track each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Primitives.Track
   }
...
New-System.Windows.Controls.Primitives.Track @PSBoundParameters
