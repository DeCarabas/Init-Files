...
,
	[Parameter(ParameterSetName='Default')]
	[Object[]]$ContentHorizontalOffset
,
	[Parameter(ParameterSetName='Default')]
...
,
	[Parameter(ParameterSetName='Default')]
	[Switch]$IsDeferredScrollingEnabled
,
	[Parameter(ParameterSetName='Default')]
	[Switch]$IsEnabled
,
...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ScrollViewer not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ScrollViewer in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.ScrollViewer {
<#
.Synopsis
   Create a new ScrollViewer object
.Description
   Generates a new System.Windows.Controls.ScrollViewer object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.ScrollViewer
   $All = Get-Parameter New-System.Windows.Controls.ScrollViewer | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.ScrollViewer is not a collection
   # So if we're in a pipeline, make a new ScrollViewer each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.ScrollViewer
   }
...
New-System.Windows.Controls.ScrollViewer @PSBoundParameters
