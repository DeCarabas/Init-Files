...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TriggerActionCollection not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TriggerActionCollection in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.TriggerActionCollection {
<#
.Synopsis
   Create a new TriggerActionCollection object
.Description
   Generates a new System.Windows.TriggerActionCollection object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.TriggerActionCollection
   $All = Get-Parameter New-System.Windows.TriggerActionCollection | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.TriggerActionCollection is not a collection
   # So if we're in a pipeline, make a new TriggerActionCollection each time
   if($_) { 
      $DObject = New-Object System.Windows.TriggerActionCollection
   }
...
New-System.Windows.TriggerActionCollection @PSBoundParameters
