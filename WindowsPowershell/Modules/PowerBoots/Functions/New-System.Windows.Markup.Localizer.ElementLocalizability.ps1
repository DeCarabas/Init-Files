...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ElementLocalizability not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ElementLocalizability in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.Localizer.ElementLocalizability {
<#
.Synopsis
   Create a new ElementLocalizability object
.Description
   Generates a new System.Windows.Markup.Localizer.ElementLocalizability object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.Localizer.ElementLocalizability
   $All = Get-Parameter New-System.Windows.Markup.Localizer.ElementLocalizability | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.Localizer.ElementLocalizability is not a collection
   # So if we're in a pipeline, make a new ElementLocalizability each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.Localizer.ElementLocalizability
   }
...
New-System.Windows.Markup.Localizer.ElementLocalizability @PSBoundParameters
