...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "NullExtension not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "NullExtension in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.NullExtension {
<#
.Synopsis
   Create a new NullExtension object
.Description
   Generates a new System.Windows.Markup.NullExtension object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.NullExtension
   $All = Get-Parameter New-System.Windows.Markup.NullExtension | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.NullExtension is not a collection
   # So if we're in a pipeline, make a new NullExtension each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.NullExtension
   }
...
New-System.Windows.Markup.NullExtension @PSBoundParameters
