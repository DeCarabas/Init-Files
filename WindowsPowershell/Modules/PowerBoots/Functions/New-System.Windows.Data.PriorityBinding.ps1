...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "PriorityBinding not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "PriorityBinding in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Data.PriorityBinding {
<#
.Synopsis
   Create a new PriorityBinding object
.Description
   Generates a new System.Windows.Data.PriorityBinding object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Data.PriorityBinding
   $All = Get-Parameter New-System.Windows.Data.PriorityBinding | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Data.PriorityBinding is not a collection
   # So if we're in a pipeline, make a new PriorityBinding each time
   if($_) { 
      $DObject = New-Object System.Windows.Data.PriorityBinding
   }
...
New-System.Windows.Data.PriorityBinding @PSBoundParameters
