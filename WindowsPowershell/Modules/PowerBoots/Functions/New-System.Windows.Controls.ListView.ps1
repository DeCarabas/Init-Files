...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ListView not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ListView in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.ListView {
<#
.Synopsis
   Create a new ListView object
.Description
   Generates a new System.Windows.Controls.ListView object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.ListView
   $All = Get-Parameter New-System.Windows.Controls.ListView | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Controls.ListView @PSBoundParameters
