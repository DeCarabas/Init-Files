...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Style not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Style in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Style {
<#
.Synopsis
   Create a new Style object
.Description
   Generates a new System.Windows.Style object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Style
   $All = Get-Parameter New-System.Windows.Style | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Style is not a collection
   # So if we're in a pipeline, make a new Style each time
   if($_) { 
      $DObject = New-Object System.Windows.Style
   }
...
New-System.Windows.Style @PSBoundParameters
