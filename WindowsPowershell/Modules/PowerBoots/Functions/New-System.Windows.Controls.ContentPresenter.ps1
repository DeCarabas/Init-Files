...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ContentPresenter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ContentPresenter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.ContentPresenter {
<#
.Synopsis
   Create a new ContentPresenter object
.Description
   Generates a new System.Windows.Controls.ContentPresenter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.ContentPresenter
   $All = Get-Parameter New-System.Windows.Controls.ContentPresenter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.ContentPresenter is not a collection
   # So if we're in a pipeline, make a new ContentPresenter each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.ContentPresenter
   }
...
New-System.Windows.Controls.ContentPresenter @PSBoundParameters
