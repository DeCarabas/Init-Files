...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "FrameworkContentElement not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "FrameworkContentElement in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.FrameworkContentElement {
<#
.Synopsis
   Create a new FrameworkContentElement object
.Description
   Generates a new System.Windows.FrameworkContentElement object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.FrameworkContentElement
   $All = Get-Parameter New-System.Windows.FrameworkContentElement | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.FrameworkContentElement is not a collection
   # So if we're in a pipeline, make a new FrameworkContentElement each time
   if($_) { 
      $DObject = New-Object System.Windows.FrameworkContentElement
   }
...
New-System.Windows.FrameworkContentElement @PSBoundParameters
