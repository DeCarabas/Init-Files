...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "SetterBaseCollection not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "SetterBaseCollection in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.SetterBaseCollection {
<#
.Synopsis
   Create a new SetterBaseCollection object
.Description
   Generates a new System.Windows.SetterBaseCollection object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.SetterBaseCollection
   $All = Get-Parameter New-System.Windows.SetterBaseCollection | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.SetterBaseCollection is not a collection
   # So if we're in a pipeline, make a new SetterBaseCollection each time
   if($_) { 
      $DObject = New-Object System.Windows.SetterBaseCollection
   }
...
New-System.Windows.SetterBaseCollection @PSBoundParameters
