...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "SetStoryboardSpeedRatio not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "SetStoryboardSpeedRatio in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Animation.SetStoryboardSpeedRatio {
<#
.Synopsis
   Create a new SetStoryboardSpeedRatio object
.Description
   Generates a new System.Windows.Media.Animation.SetStoryboardSpeedRatio object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Animation.SetStoryboardSpeedRatio
   $All = Get-Parameter New-System.Windows.Media.Animation.SetStoryboardSpeedRatio | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Animation.SetStoryboardSpeedRatio is not a collection
   # So if we're in a pipeline, make a new SetStoryboardSpeedRatio each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Animation.SetStoryboardSpeedRatio
   }
...
New-System.Windows.Media.Animation.SetStoryboardSpeedRatio @PSBoundParameters
