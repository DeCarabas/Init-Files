...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ParserContext not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ParserContext in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.ParserContext {
<#
.Synopsis
   Create a new ParserContext object
.Description
   Generates a new System.Windows.Markup.ParserContext object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.ParserContext
   $All = Get-Parameter New-System.Windows.Markup.ParserContext | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.ParserContext is not a collection
   # So if we're in a pipeline, make a new ParserContext each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.ParserContext
   }
...
New-System.Windows.Markup.ParserContext @PSBoundParameters
