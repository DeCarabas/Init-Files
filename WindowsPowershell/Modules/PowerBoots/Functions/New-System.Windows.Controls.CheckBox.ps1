...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "CheckBox not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "CheckBox in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.CheckBox {
<#
.Synopsis
   Create a new CheckBox object
.Description
   Generates a new System.Windows.Controls.CheckBox object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.CheckBox
   $All = Get-Parameter New-System.Windows.Controls.CheckBox | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.CheckBox is not a collection
   # So if we're in a pipeline, make a new CheckBox each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.CheckBox
   }
...
New-System.Windows.Controls.CheckBox @PSBoundParameters
