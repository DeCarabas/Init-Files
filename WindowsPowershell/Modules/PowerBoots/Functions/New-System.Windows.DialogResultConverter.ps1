...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "DialogResultConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "DialogResultConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.DialogResultConverter {
<#
.Synopsis
   Create a new DialogResultConverter object
.Description
   Generates a new System.Windows.DialogResultConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.DialogResultConverter
   $All = Get-Parameter New-System.Windows.DialogResultConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.DialogResultConverter is not a collection
   # So if we're in a pipeline, make a new DialogResultConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.DialogResultConverter
   }
...
New-System.Windows.DialogResultConverter @PSBoundParameters
