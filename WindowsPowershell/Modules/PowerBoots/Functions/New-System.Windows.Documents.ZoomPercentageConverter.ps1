...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ZoomPercentageConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ZoomPercentageConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.ZoomPercentageConverter {
<#
.Synopsis
   Create a new ZoomPercentageConverter object
.Description
   Generates a new System.Windows.Documents.ZoomPercentageConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.ZoomPercentageConverter
   $All = Get-Parameter New-System.Windows.Documents.ZoomPercentageConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.ZoomPercentageConverter is not a collection
   # So if we're in a pipeline, make a new ZoomPercentageConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.ZoomPercentageConverter
   }
...
New-System.Windows.Documents.ZoomPercentageConverter @PSBoundParameters
