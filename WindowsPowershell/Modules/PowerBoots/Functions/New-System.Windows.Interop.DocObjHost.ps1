...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "DocObjHost not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "DocObjHost in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Interop.DocObjHost {
<#
.Synopsis
   Create a new DocObjHost object
.Description
   Generates a new System.Windows.Interop.DocObjHost object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Interop.DocObjHost
   $All = Get-Parameter New-System.Windows.Interop.DocObjHost | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Interop.DocObjHost is not a collection
   # So if we're in a pipeline, make a new DocObjHost each time
   if($_) { 
      $DObject = New-Object System.Windows.Interop.DocObjHost
   }
...
New-System.Windows.Interop.DocObjHost @PSBoundParameters
