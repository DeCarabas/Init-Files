...
,
	[Parameter(ParameterSetName='Default')]
	[Object[]]$CustomPopupPlacementCallback
,
	[Parameter(ParameterSetName='Default')]
	[Object[]]$DataContext
,
...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Popup not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Popup in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Primitives.Popup {
<#
.Synopsis
   Create a new Popup object
.Description
   Generates a new System.Windows.Controls.Primitives.Popup object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Primitives.Popup
   $All = Get-Parameter New-System.Windows.Controls.Primitives.Popup | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Primitives.Popup is not a collection
   # So if we're in a pipeline, make a new Popup each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Primitives.Popup
   }
...
New-System.Windows.Controls.Primitives.Popup @PSBoundParameters
