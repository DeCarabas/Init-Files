...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ContentTypes not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ContentTypes in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Resources.ContentTypes {
<#
.Synopsis
   Create a new ContentTypes object
.Description
   Generates a new System.Windows.Resources.ContentTypes object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Resources.ContentTypes
   $All = Get-Parameter New-System.Windows.Resources.ContentTypes | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Resources.ContentTypes is not a collection
   # So if we're in a pipeline, make a new ContentTypes each time
   if($_) { 
      $DObject = New-Object System.Windows.Resources.ContentTypes
   }
...
New-System.Windows.Resources.ContentTypes @PSBoundParameters
