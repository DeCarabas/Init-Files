...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Polygon not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Polygon in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Shapes.Polygon {
<#
.Synopsis
   Create a new Polygon object
.Description
   Generates a new System.Windows.Shapes.Polygon object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Shapes.Polygon
   $All = Get-Parameter New-System.Windows.Shapes.Polygon | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Shapes.Polygon is not a collection
   # So if we're in a pipeline, make a new Polygon each time
   if($_) { 
      $DObject = New-Object System.Windows.Shapes.Polygon
   }
...
New-System.Windows.Shapes.Polygon @PSBoundParameters
