...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "PasswordBox not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "PasswordBox in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.PasswordBox {
<#
.Synopsis
   Create a new PasswordBox object
.Description
   Generates a new System.Windows.Controls.PasswordBox object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.PasswordBox
   $All = Get-Parameter New-System.Windows.Controls.PasswordBox | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.PasswordBox is not a collection
   # So if we're in a pipeline, make a new PasswordBox each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.PasswordBox
   }
...
New-System.Windows.Controls.PasswordBox @PSBoundParameters
