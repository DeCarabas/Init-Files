...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Border not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Border in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Border {
<#
.Synopsis
   Create a new Border object
.Description
   Generates a new System.Windows.Controls.Border object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Border
   $All = Get-Parameter New-System.Windows.Controls.Border | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Border is not a collection
   # So if we're in a pipeline, make a new Border each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Border
   }
...
New-System.Windows.Controls.Border @PSBoundParameters
