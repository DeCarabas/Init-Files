...
(defvar font-lock-defaults-alist
  (let (;; We use `beginning-of-defun', rather than nil, for SYNTAX-BEGIN.
    ...
    (list
     ...
     (cons 'csharp-mode			csharp-mode-defaults)
...
;	     "virtual" "override" "abstract" "extern" "internal"
	...
	(eval-when-compile
	  (regexp-opt
	   '("+" "-" "*" "/" "%" "^" "&" "|" "~" "!" "=" "<" ">" "+=" "-="
	     "*=" "/=" "%=" "^=" "&=" "|=" "<<" ">>" ">>=" "<<=" "==" "!="
	     "<=" ">=" "&&" "||" "++" "--" "->"
             "is" "as" "typeof" "sizeof" "new" "checked" "unchecked"
             ))))
       ;;
       ;; Classes immediately followed by an object name.
       (csharp-type-names
	  ...
	  (cons 
	   (,@ (eval-when-compile
		 (regexp-opt '("bool" "char" "byte"
			       ...
			       "float" "double" "void"
			       "enum" "string" "delegate" "object" "event"
			       ))))
	...
	(eval-when-compile
	  (regexp-opt
	   '(
	     ...
	     "interface"
	     "in" "out" "ref"
	     "virtual" "override" "abstract" "extern" "struct"
	     ))))
 ...
 (setq csharp-font-lock-keywords-1
  (list
   ...
   ;; Fontify namespace names in using directives.
   '("\\<\\(using\\)\\>\\s +\\(\\(\\sw\\|\\.\\)+\\)?"
     (1 font-lock-keyword-face)
     (2 font-lock-constant-face nil t)
     ("\\s *=\\s *\\(\\(\\sw\\|\\.\\)+\\)\\s *;" nil nil
 ...
 (setq csharp-font-lock-keywords-2
  (append csharp-font-lock-keywords-1
   (list
    ...
    ;; xmldoc tags within comments.
    ;; form is <tag> or </tag>
    (list
     (concat "\\("
             "listheader\\|term\\|description\\|"
             "c\\|code\\|example\\|"
	     "para\\|remarks\\|returns\\|"
	     "summary\\|value"
	     "\\)>")
     '(1 font-lock-constant-face prepend))

     ;; form is:
     ;; <tag attr="val">desc</tag>
    (list
     (concat 
             "<"
             "\\(param\\|exception\\|permission\\)"
             "\\s +"
             "\\(\\sw+\\)="
             "\""
             "\\([^\\\"]+\\)"
    ...
    ;; JohnDoty: Commented this out, because it mistakes arrays as attributes.
    ;;           Dumbass.
    ;;
    ;; pretty up the C# attributes we can recognize
    ;;
    ;; (list
    ;; "\\(\\[\\)\\s *\\(\\(\\sw\\|\\.\\)+\\)([^)]*)\\s *\\(\\]\\)"
    ;; '(1 font-lock-warning-face prepend)
    ;; '(2 font-lock-constant-face prepend)
    ;; '(4 font-lock-warning-face prepend)
    ;; )
