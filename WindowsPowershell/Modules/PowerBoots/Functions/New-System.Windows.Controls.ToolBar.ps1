...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ToolBar not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ToolBar in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.ToolBar {
<#
.Synopsis
   Create a new ToolBar object
.Description
   Generates a new System.Windows.Controls.ToolBar object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.ToolBar
   $All = Get-Parameter New-System.Windows.Controls.ToolBar | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Controls.ToolBar @PSBoundParameters
