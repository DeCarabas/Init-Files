...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ListStructure not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ListStructure in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentStructures.ListStructure {
<#
.Synopsis
   Create a new ListStructure object
.Description
   Generates a new System.Windows.Documents.DocumentStructures.ListStructure object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentStructures.ListStructure
   $All = Get-Parameter New-System.Windows.Documents.DocumentStructures.ListStructure | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentStructures.ListStructure is not a collection
   # So if we're in a pipeline, make a new ListStructure each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentStructures.ListStructure
   }
...
New-System.Windows.Documents.DocumentStructures.ListStructure @PSBoundParameters
