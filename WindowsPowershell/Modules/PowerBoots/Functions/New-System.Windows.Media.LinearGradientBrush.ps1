...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "LinearGradientBrush not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "LinearGradientBrush in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.LinearGradientBrush {
<#
.Synopsis
   Create a new LinearGradientBrush object
.Description
   Generates a new System.Windows.Media.LinearGradientBrush object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.LinearGradientBrush
   $All = Get-Parameter New-System.Windows.Media.LinearGradientBrush | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Media.LinearGradientBrush @PSBoundParameters
