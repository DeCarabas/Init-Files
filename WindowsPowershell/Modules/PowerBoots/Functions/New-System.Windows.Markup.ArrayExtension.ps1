...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ArrayExtension not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ArrayExtension in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.ArrayExtension {
<#
.Synopsis
   Create a new ArrayExtension object
.Description
   Generates a new System.Windows.Markup.ArrayExtension object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.ArrayExtension
   $All = Get-Parameter New-System.Windows.Markup.ArrayExtension | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.ArrayExtension is not a collection
   # So if we're in a pipeline, make a new ArrayExtension each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.ArrayExtension
   }
...
New-System.Windows.Markup.ArrayExtension @PSBoundParameters
