...
PARAM(
	...
	[Switch]$IsAsynchronous
,
	[Parameter(ParameterSetName='Default')]
	[Switch]$IsInitialLoadEnabled
,
	[Parameter(ParameterSetName='Default')]
	[Object[]]$MethodName
,
...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ObjectDataProvider not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ObjectDataProvider in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Data.ObjectDataProvider {
<#
.Synopsis
   Create a new ObjectDataProvider object
.Description
   Generates a new System.Windows.Data.ObjectDataProvider object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Data.ObjectDataProvider
   $All = Get-Parameter New-System.Windows.Data.ObjectDataProvider | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Data.ObjectDataProvider is not a collection
   # So if we're in a pipeline, make a new ObjectDataProvider each time
   if($_) { 
      $DObject = New-Object System.Windows.Data.ObjectDataProvider
   }
...
New-System.Windows.Data.ObjectDataProvider @PSBoundParameters
