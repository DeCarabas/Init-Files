...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Button not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Button in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Button {
<#
.Synopsis
   Create a new Button object
.Description
   Generates a new System.Windows.Controls.Button object, and allows setting all of it's properties
.Notes
...
BEGIN {
   ...
   $All = Get-Parameter New-System.Windows.Controls.Button | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Button is not a collection
   # So if we're in a pipeline, make a new Button each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Button
   }
...
New-System.Windows.Controls.Button @PSBoundParameters
