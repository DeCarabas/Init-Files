...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "DynamicResourceExtensionConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "DynamicResourceExtensionConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.DynamicResourceExtensionConverter {
<#
.Synopsis
   Create a new DynamicResourceExtensionConverter object
.Description
   Generates a new System.Windows.DynamicResourceExtensionConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.DynamicResourceExtensionConverter
   $All = Get-Parameter New-System.Windows.DynamicResourceExtensionConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.DynamicResourceExtensionConverter is not a collection
   # So if we're in a pipeline, make a new DynamicResourceExtensionConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.DynamicResourceExtensionConverter
   }
...
New-System.Windows.DynamicResourceExtensionConverter @PSBoundParameters
