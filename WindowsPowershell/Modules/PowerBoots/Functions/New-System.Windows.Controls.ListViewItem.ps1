...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ListViewItem not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ListViewItem in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.ListViewItem {
<#
.Synopsis
   Create a new ListViewItem object
.Description
   Generates a new System.Windows.Controls.ListViewItem object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.ListViewItem
   $All = Get-Parameter New-System.Windows.Controls.ListViewItem | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.ListViewItem is not a collection
   # So if we're in a pipeline, make a new ListViewItem each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.ListViewItem
   }
...
New-System.Windows.Controls.ListViewItem @PSBoundParameters
