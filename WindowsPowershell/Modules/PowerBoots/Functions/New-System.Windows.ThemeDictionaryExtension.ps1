...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ThemeDictionaryExtension not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ThemeDictionaryExtension in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.ThemeDictionaryExtension {
<#
.Synopsis
   Create a new ThemeDictionaryExtension object
.Description
   Generates a new System.Windows.ThemeDictionaryExtension object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.ThemeDictionaryExtension
   $All = Get-Parameter New-System.Windows.ThemeDictionaryExtension | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.ThemeDictionaryExtension is not a collection
   # So if we're in a pipeline, make a new ThemeDictionaryExtension each time
   if($_) { 
      $DObject = New-Object System.Windows.ThemeDictionaryExtension
   }
...
New-System.Windows.ThemeDictionaryExtension @PSBoundParameters
