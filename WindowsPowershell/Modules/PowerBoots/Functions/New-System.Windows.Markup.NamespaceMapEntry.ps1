...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "NamespaceMapEntry not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "NamespaceMapEntry in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.NamespaceMapEntry {
<#
.Synopsis
   Create a new NamespaceMapEntry object
.Description
   Generates a new System.Windows.Markup.NamespaceMapEntry object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.NamespaceMapEntry
   $All = Get-Parameter New-System.Windows.Markup.NamespaceMapEntry | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.NamespaceMapEntry is not a collection
   # So if we're in a pipeline, make a new NamespaceMapEntry each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.NamespaceMapEntry
   }
...
New-System.Windows.Markup.NamespaceMapEntry @PSBoundParameters
