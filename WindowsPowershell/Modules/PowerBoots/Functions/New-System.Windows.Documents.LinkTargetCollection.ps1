...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "LinkTargetCollection not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "LinkTargetCollection in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.LinkTargetCollection {
<#
.Synopsis
   Create a new LinkTargetCollection object
.Description
   Generates a new System.Windows.Documents.LinkTargetCollection object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.LinkTargetCollection
   $All = Get-Parameter New-System.Windows.Documents.LinkTargetCollection | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.LinkTargetCollection is not a collection
   # So if we're in a pipeline, make a new LinkTargetCollection each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.LinkTargetCollection
   }
...
New-System.Windows.Documents.LinkTargetCollection @PSBoundParameters
