...
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_DragStarted
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_DragDelta
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_DragCompleted
,
	[Parameter(ParameterSetName='Default')]
...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "GridSplitter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "GridSplitter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.GridSplitter {
<#
.Synopsis
   Create a new GridSplitter object
.Description
   Generates a new System.Windows.Controls.GridSplitter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.GridSplitter
   $All = Get-Parameter New-System.Windows.Controls.GridSplitter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.GridSplitter is not a collection
   # So if we're in a pipeline, make a new GridSplitter each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.GridSplitter
   }
...
New-System.Windows.Controls.GridSplitter @PSBoundParameters
