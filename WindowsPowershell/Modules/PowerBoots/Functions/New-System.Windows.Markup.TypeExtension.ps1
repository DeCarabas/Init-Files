...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TypeExtension not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TypeExtension in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.TypeExtension {
<#
.Synopsis
   Create a new TypeExtension object
.Description
   Generates a new System.Windows.Markup.TypeExtension object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.TypeExtension
   $All = Get-Parameter New-System.Windows.Markup.TypeExtension | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.TypeExtension is not a collection
   # So if we're in a pipeline, make a new TypeExtension each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.TypeExtension
   }
...
New-System.Windows.Markup.TypeExtension @PSBoundParameters
