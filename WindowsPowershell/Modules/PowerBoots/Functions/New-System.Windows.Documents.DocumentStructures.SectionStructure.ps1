...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "SectionStructure not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "SectionStructure in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentStructures.SectionStructure {
<#
.Synopsis
   Create a new SectionStructure object
.Description
   Generates a new System.Windows.Documents.DocumentStructures.SectionStructure object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentStructures.SectionStructure
   $All = Get-Parameter New-System.Windows.Documents.DocumentStructures.SectionStructure | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentStructures.SectionStructure is not a collection
   # So if we're in a pipeline, make a new SectionStructure each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentStructures.SectionStructure
   }
...
New-System.Windows.Documents.DocumentStructures.SectionStructure @PSBoundParameters
