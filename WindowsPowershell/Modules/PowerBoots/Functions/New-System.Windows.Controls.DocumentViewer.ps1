...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "DocumentViewer not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "DocumentViewer in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.DocumentViewer {
<#
.Synopsis
   Create a new DocumentViewer object
.Description
   Generates a new System.Windows.Controls.DocumentViewer object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.DocumentViewer
   $All = Get-Parameter New-System.Windows.Controls.DocumentViewer | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.DocumentViewer is not a collection
   # So if we're in a pipeline, make a new DocumentViewer each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.DocumentViewer
   }
...
New-System.Windows.Controls.DocumentViewer @PSBoundParameters
