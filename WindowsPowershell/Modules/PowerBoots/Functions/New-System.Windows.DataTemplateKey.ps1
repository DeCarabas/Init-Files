...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "DataTemplateKey not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "DataTemplateKey in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.DataTemplateKey {
<#
.Synopsis
   Create a new DataTemplateKey object
.Description
   Generates a new System.Windows.DataTemplateKey object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.DataTemplateKey
   $All = Get-Parameter New-System.Windows.DataTemplateKey | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.DataTemplateKey is not a collection
   # So if we're in a pipeline, make a new DataTemplateKey each time
   if($_) { 
      $DObject = New-Object System.Windows.DataTemplateKey
   }
...
New-System.Windows.DataTemplateKey @PSBoundParameters
