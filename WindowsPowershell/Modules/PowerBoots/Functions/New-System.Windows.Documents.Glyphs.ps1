...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Glyphs not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Glyphs in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.Glyphs {
<#
.Synopsis
   Create a new Glyphs object
.Description
   Generates a new System.Windows.Documents.Glyphs object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.Glyphs
   $All = Get-Parameter New-System.Windows.Documents.Glyphs | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.Glyphs is not a collection
   # So if we're in a pipeline, make a new Glyphs each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.Glyphs
   }
...
New-System.Windows.Documents.Glyphs @PSBoundParameters
