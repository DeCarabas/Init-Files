...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "RotateTransform not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "RotateTransform in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.RotateTransform {
<#
.Synopsis
   Create a new RotateTransform object
.Description
   Generates a new System.Windows.Media.RotateTransform object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.RotateTransform
   $All = Get-Parameter New-System.Windows.Media.RotateTransform | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.RotateTransform is not a collection
   # So if we're in a pipeline, make a new RotateTransform each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.RotateTransform
   }
...
New-System.Windows.Media.RotateTransform @PSBoundParameters
