...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "CollectionContainer not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "CollectionContainer in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Data.CollectionContainer {
<#
.Synopsis
   Create a new CollectionContainer object
.Description
   Generates a new System.Windows.Data.CollectionContainer object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Data.CollectionContainer
   $All = Get-Parameter New-System.Windows.Data.CollectionContainer | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Data.CollectionContainer is not a collection
   # So if we're in a pipeline, make a new CollectionContainer each time
   if($_) { 
      $DObject = New-Object System.Windows.Data.CollectionContainer
   }
...
New-System.Windows.Data.CollectionContainer @PSBoundParameters
