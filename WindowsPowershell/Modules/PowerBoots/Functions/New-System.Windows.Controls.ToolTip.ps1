...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ToolTip not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ToolTip in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.ToolTip {
<#
.Synopsis
   Create a new ToolTip object
.Description
   Generates a new System.Windows.Controls.ToolTip object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.ToolTip
   $All = Get-Parameter New-System.Windows.Controls.ToolTip | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.ToolTip is not a collection
   # So if we're in a pipeline, make a new ToolTip each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.ToolTip
   }
...
New-System.Windows.Controls.ToolTip @PSBoundParameters
