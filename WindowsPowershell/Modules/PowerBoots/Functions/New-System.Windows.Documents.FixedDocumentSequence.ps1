...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "FixedDocumentSequence not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "FixedDocumentSequence in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.FixedDocumentSequence {
<#
.Synopsis
   Create a new FixedDocumentSequence object
.Description
   Generates a new System.Windows.Documents.FixedDocumentSequence object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.FixedDocumentSequence
   $All = Get-Parameter New-System.Windows.Documents.FixedDocumentSequence | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.FixedDocumentSequence is not a collection
   # So if we're in a pipeline, make a new FixedDocumentSequence each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.FixedDocumentSequence
   }
...
New-System.Windows.Documents.FixedDocumentSequence @PSBoundParameters
