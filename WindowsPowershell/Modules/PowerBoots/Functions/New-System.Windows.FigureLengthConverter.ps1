...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "FigureLengthConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "FigureLengthConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.FigureLengthConverter {
<#
.Synopsis
   Create a new FigureLengthConverter object
.Description
   Generates a new System.Windows.FigureLengthConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.FigureLengthConverter
   $All = Get-Parameter New-System.Windows.FigureLengthConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.FigureLengthConverter is not a collection
   # So if we're in a pipeline, make a new FigureLengthConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.FigureLengthConverter
   }
...
New-System.Windows.FigureLengthConverter @PSBoundParameters
