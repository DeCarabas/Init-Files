...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "UserControl not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "UserControl in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.UserControl {
<#
.Synopsis
   Create a new UserControl object
.Description
   Generates a new System.Windows.Controls.UserControl object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.UserControl
   $All = Get-Parameter New-System.Windows.Controls.UserControl | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.UserControl is not a collection
   # So if we're in a pipeline, make a new UserControl each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.UserControl
   }
...
New-System.Windows.Controls.UserControl @PSBoundParameters
