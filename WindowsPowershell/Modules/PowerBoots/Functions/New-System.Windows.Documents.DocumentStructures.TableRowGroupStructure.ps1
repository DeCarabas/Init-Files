...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TableRowGroupStructure not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TableRowGroupStructure in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentStructures.TableRowGroupStructure {
<#
.Synopsis
   Create a new TableRowGroupStructure object
.Description
   Generates a new System.Windows.Documents.DocumentStructures.TableRowGroupStructure object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentStructures.TableRowGroupStructure
   $All = Get-Parameter New-System.Windows.Documents.DocumentStructures.TableRowGroupStructure | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentStructures.TableRowGroupStructure is not a collection
   # So if we're in a pipeline, make a new TableRowGroupStructure each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentStructures.TableRowGroupStructure
   }
...
New-System.Windows.Documents.DocumentStructures.TableRowGroupStructure @PSBoundParameters
