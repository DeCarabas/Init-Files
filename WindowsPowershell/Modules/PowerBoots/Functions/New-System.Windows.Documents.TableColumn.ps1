...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TableColumn not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TableColumn in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.TableColumn {
<#
.Synopsis
   Create a new TableColumn object
.Description
   Generates a new System.Windows.Documents.TableColumn object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.TableColumn
   $All = Get-Parameter New-System.Windows.Documents.TableColumn | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.TableColumn is not a collection
   # So if we're in a pipeline, make a new TableColumn each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.TableColumn
   }
...
New-System.Windows.Documents.TableColumn @PSBoundParameters
