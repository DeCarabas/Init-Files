...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TemplatePartAttribute not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TemplatePartAttribute in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.TemplatePartAttribute {
<#
.Synopsis
   Create a new TemplatePartAttribute object
.Description
   Generates a new System.Windows.TemplatePartAttribute object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.TemplatePartAttribute
   $All = Get-Parameter New-System.Windows.TemplatePartAttribute | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.TemplatePartAttribute is not a collection
   # So if we're in a pipeline, make a new TemplatePartAttribute each time
   if($_) { 
      $DObject = New-Object System.Windows.TemplatePartAttribute
   }
...
New-System.Windows.TemplatePartAttribute @PSBoundParameters
