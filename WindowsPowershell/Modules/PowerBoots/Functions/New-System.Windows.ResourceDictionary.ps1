...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ResourceDictionary not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ResourceDictionary in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.ResourceDictionary {
<#
.Synopsis
   Create a new ResourceDictionary object
.Description
   Generates a new System.Windows.ResourceDictionary object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.ResourceDictionary
   $All = Get-Parameter New-System.Windows.ResourceDictionary | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.ResourceDictionary is not a collection
   # So if we're in a pipeline, make a new ResourceDictionary each time
   if($_) { 
      $DObject = New-Object System.Windows.ResourceDictionary
   }
...
New-System.Windows.ResourceDictionary @PSBoundParameters
