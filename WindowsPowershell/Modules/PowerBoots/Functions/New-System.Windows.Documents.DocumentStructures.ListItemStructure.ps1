...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ListItemStructure not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ListItemStructure in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentStructures.ListItemStructure {
<#
.Synopsis
   Create a new ListItemStructure object
.Description
   Generates a new System.Windows.Documents.DocumentStructures.ListItemStructure object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentStructures.ListItemStructure
   $All = Get-Parameter New-System.Windows.Documents.DocumentStructures.ListItemStructure | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentStructures.ListItemStructure is not a collection
   # So if we're in a pipeline, make a new ListItemStructure each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentStructures.ListItemStructure
   }
...
New-System.Windows.Documents.DocumentStructures.ListItemStructure @PSBoundParameters
