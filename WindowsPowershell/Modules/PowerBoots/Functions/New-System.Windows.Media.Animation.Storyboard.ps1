...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Storyboard not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Storyboard in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Animation.Storyboard {
<#
.Synopsis
   Create a new Storyboard object
.Description
   Generates a new System.Windows.Media.Animation.Storyboard object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Animation.Storyboard
   $All = Get-Parameter New-System.Windows.Media.Animation.Storyboard | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Media.Animation.Storyboard @PSBoundParameters
