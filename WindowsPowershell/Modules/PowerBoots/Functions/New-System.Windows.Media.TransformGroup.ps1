...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TransformGroup not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TransformGroup in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.TransformGroup {
<#
.Synopsis
   Create a new TransformGroup object
.Description
   Generates a new System.Windows.Media.TransformGroup object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.TransformGroup
   $All = Get-Parameter New-System.Windows.Media.TransformGroup | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Media.TransformGroup @PSBoundParameters
