...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TableCell not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TableCell in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.TableCell {
<#
.Synopsis
   Create a new TableCell object
.Description
   Generates a new System.Windows.Documents.TableCell object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.TableCell
   $All = Get-Parameter New-System.Windows.Documents.TableCell | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Documents.TableCell @PSBoundParameters
