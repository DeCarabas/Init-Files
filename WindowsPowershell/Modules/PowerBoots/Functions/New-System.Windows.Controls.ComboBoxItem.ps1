...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ComboBoxItem not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ComboBoxItem in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.ComboBoxItem {
<#
.Synopsis
   Create a new ComboBoxItem object
.Description
   Generates a new System.Windows.Controls.ComboBoxItem object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.ComboBoxItem
   $All = Get-Parameter New-System.Windows.Controls.ComboBoxItem | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.ComboBoxItem is not a collection
   # So if we're in a pipeline, make a new ComboBoxItem each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.ComboBoxItem
   }
...
New-System.Windows.Controls.ComboBoxItem @PSBoundParameters
