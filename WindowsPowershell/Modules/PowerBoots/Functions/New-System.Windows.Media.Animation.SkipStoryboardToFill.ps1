...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "SkipStoryboardToFill not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "SkipStoryboardToFill in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Animation.SkipStoryboardToFill {
<#
.Synopsis
   Create a new SkipStoryboardToFill object
.Description
   Generates a new System.Windows.Media.Animation.SkipStoryboardToFill object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Animation.SkipStoryboardToFill
   $All = Get-Parameter New-System.Windows.Media.Animation.SkipStoryboardToFill | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Animation.SkipStoryboardToFill is not a collection
   # So if we're in a pipeline, make a new SkipStoryboardToFill each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Animation.SkipStoryboardToFill
   }
...
New-System.Windows.Media.Animation.SkipStoryboardToFill @PSBoundParameters
