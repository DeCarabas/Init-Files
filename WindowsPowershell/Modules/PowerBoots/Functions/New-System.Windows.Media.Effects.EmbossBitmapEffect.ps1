...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "EmbossBitmapEffect not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "EmbossBitmapEffect in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Effects.EmbossBitmapEffect {
<#
.Synopsis
   Create a new EmbossBitmapEffect object
.Description
   Generates a new System.Windows.Media.Effects.EmbossBitmapEffect object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Effects.EmbossBitmapEffect
   $All = Get-Parameter New-System.Windows.Media.Effects.EmbossBitmapEffect | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Effects.EmbossBitmapEffect is not a collection
   # So if we're in a pipeline, make a new EmbossBitmapEffect each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Effects.EmbossBitmapEffect
   }
...
New-System.Windows.Media.Effects.EmbossBitmapEffect @PSBoundParameters
