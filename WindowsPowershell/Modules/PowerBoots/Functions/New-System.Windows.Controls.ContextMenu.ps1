...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ContextMenu not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ContextMenu in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.ContextMenu {
<#
.Synopsis
   Create a new ContextMenu object
.Description
   Generates a new System.Windows.Controls.ContextMenu object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.ContextMenu
   $All = Get-Parameter New-System.Windows.Controls.ContextMenu | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Controls.ContextMenu @PSBoundParameters
