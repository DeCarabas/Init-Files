...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "StatusBarItem not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "StatusBarItem in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Primitives.StatusBarItem {
<#
.Synopsis
   Create a new StatusBarItem object
.Description
   Generates a new System.Windows.Controls.Primitives.StatusBarItem object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Primitives.StatusBarItem
   $All = Get-Parameter New-System.Windows.Controls.Primitives.StatusBarItem | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Primitives.StatusBarItem is not a collection
   # So if we're in a pipeline, make a new StatusBarItem each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Primitives.StatusBarItem
   }
...
New-System.Windows.Controls.Primitives.StatusBarItem @PSBoundParameters
