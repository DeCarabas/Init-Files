...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "GradientStop not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "GradientStop in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.GradientStop {
<#
.Synopsis
   Create a new GradientStop object
.Description
   Generates a new System.Windows.Media.GradientStop object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.GradientStop
   $All = Get-Parameter New-System.Windows.Media.GradientStop | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.GradientStop is not a collection
   # So if we're in a pipeline, make a new GradientStop each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.GradientStop
   }
...
New-System.Windows.Media.GradientStop @PSBoundParameters
