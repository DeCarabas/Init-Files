...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "BevelBitmapEffect not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "BevelBitmapEffect in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Effects.BevelBitmapEffect {
<#
.Synopsis
   Create a new BevelBitmapEffect object
.Description
   Generates a new System.Windows.Media.Effects.BevelBitmapEffect object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Effects.BevelBitmapEffect
   $All = Get-Parameter New-System.Windows.Media.Effects.BevelBitmapEffect | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Effects.BevelBitmapEffect is not a collection
   # So if we're in a pipeline, make a new BevelBitmapEffect each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Effects.BevelBitmapEffect
   }
...
New-System.Windows.Media.Effects.BevelBitmapEffect @PSBoundParameters
