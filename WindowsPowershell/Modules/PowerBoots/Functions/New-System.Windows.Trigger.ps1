...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Trigger not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Trigger in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Trigger {
<#
.Synopsis
   Create a new Trigger object
.Description
   Generates a new System.Windows.Trigger object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Trigger
   $All = Get-Parameter New-System.Windows.Trigger | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Trigger is not a collection
   # So if we're in a pipeline, make a new Trigger each time
   if($_) { 
      $DObject = New-Object System.Windows.Trigger
   }
...
New-System.Windows.Trigger @PSBoundParameters
