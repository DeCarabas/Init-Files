...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "NameScope not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "NameScope in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.NameScope {
<#
.Synopsis
   Create a new NameScope object
.Description
   Generates a new System.Windows.NameScope object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.NameScope
   $All = Get-Parameter New-System.Windows.NameScope | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.NameScope is not a collection
   # So if we're in a pipeline, make a new NameScope each time
   if($_) { 
      $DObject = New-Object System.Windows.NameScope
   }
...
New-System.Windows.NameScope @PSBoundParameters
