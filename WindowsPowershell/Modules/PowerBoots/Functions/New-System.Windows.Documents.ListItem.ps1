...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ListItem not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ListItem in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.ListItem {
<#
.Synopsis
   Create a new ListItem object
.Description
   Generates a new System.Windows.Documents.ListItem object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.ListItem
   $All = Get-Parameter New-System.Windows.Documents.ListItem | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Documents.ListItem @PSBoundParameters
