...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "RadioButton not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "RadioButton in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.RadioButton {
<#
.Synopsis
   Create a new RadioButton object
.Description
   Generates a new System.Windows.Controls.RadioButton object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.RadioButton
   $All = Get-Parameter New-System.Windows.Controls.RadioButton | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.RadioButton is not a collection
   # So if we're in a pipeline, make a new RadioButton each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.RadioButton
   }
...
New-System.Windows.Controls.RadioButton @PSBoundParameters
