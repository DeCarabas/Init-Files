...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Span not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Span in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.Span {
<#
.Synopsis
   Create a new Span object
.Description
   Generates a new System.Windows.Documents.Span object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.Span
   $All = Get-Parameter New-System.Windows.Documents.Span | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Documents.Span @PSBoundParameters
