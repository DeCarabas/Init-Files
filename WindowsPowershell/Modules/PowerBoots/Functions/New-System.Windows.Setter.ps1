...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Setter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Setter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Setter {
<#
.Synopsis
   Create a new Setter object
.Description
   Generates a new System.Windows.Setter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Setter
   $All = Get-Parameter New-System.Windows.Setter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Setter is not a collection
   # So if we're in a pipeline, make a new Setter each time
   if($_) { 
      $DObject = New-Object System.Windows.Setter
   }
...
New-System.Windows.Setter @PSBoundParameters
