...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "StaticResourceExtension not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "StaticResourceExtension in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.StaticResourceExtension {
<#
.Synopsis
   Create a new StaticResourceExtension object
.Description
   Generates a new System.Windows.StaticResourceExtension object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.StaticResourceExtension
   $All = Get-Parameter New-System.Windows.StaticResourceExtension | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.StaticResourceExtension is not a collection
   # So if we're in a pipeline, make a new StaticResourceExtension each time
   if($_) { 
      $DObject = New-Object System.Windows.StaticResourceExtension
   }
...
New-System.Windows.StaticResourceExtension @PSBoundParameters
