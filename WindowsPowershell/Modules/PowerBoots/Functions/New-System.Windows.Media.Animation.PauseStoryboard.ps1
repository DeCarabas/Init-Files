...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "PauseStoryboard not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "PauseStoryboard in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Animation.PauseStoryboard {
<#
.Synopsis
   Create a new PauseStoryboard object
.Description
   Generates a new System.Windows.Media.Animation.PauseStoryboard object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Animation.PauseStoryboard
   $All = Get-Parameter New-System.Windows.Media.Animation.PauseStoryboard | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Animation.PauseStoryboard is not a collection
   # So if we're in a pipeline, make a new PauseStoryboard each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Animation.PauseStoryboard
   }
...
New-System.Windows.Media.Animation.PauseStoryboard @PSBoundParameters
