...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TemplateBindingExtensionConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TemplateBindingExtensionConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.TemplateBindingExtensionConverter {
<#
.Synopsis
   Create a new TemplateBindingExtensionConverter object
.Description
   Generates a new System.Windows.TemplateBindingExtensionConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.TemplateBindingExtensionConverter
   $All = Get-Parameter New-System.Windows.TemplateBindingExtensionConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.TemplateBindingExtensionConverter is not a collection
   # So if we're in a pipeline, make a new TemplateBindingExtensionConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.TemplateBindingExtensionConverter
   }
...
New-System.Windows.TemplateBindingExtensionConverter @PSBoundParameters
