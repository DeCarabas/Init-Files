...
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_GetPageRootCompleted
,
	[Parameter(ParameterSetName='Default')]
...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "PageContent not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "PageContent in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.PageContent {
<#
.Synopsis
   Create a new PageContent object
.Description
   Generates a new System.Windows.Documents.PageContent object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.PageContent
   $All = Get-Parameter New-System.Windows.Documents.PageContent | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.PageContent is not a collection
   # So if we're in a pipeline, make a new PageContent each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.PageContent
   }
...
New-System.Windows.Documents.PageContent @PSBoundParameters
