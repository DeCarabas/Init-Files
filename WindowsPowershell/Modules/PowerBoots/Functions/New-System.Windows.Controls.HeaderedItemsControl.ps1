...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "HeaderedItemsControl not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "HeaderedItemsControl in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.HeaderedItemsControl {
<#
.Synopsis
   Create a new HeaderedItemsControl object
.Description
   Generates a new System.Windows.Controls.HeaderedItemsControl object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.HeaderedItemsControl
   $All = Get-Parameter New-System.Windows.Controls.HeaderedItemsControl | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Controls.HeaderedItemsControl @PSBoundParameters
