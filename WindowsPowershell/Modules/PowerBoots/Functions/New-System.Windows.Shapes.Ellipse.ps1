...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Ellipse not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Ellipse in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Shapes.Ellipse {
<#
.Synopsis
   Create a new Ellipse object
.Description
   Generates a new System.Windows.Shapes.Ellipse object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Shapes.Ellipse
   $All = Get-Parameter New-System.Windows.Shapes.Ellipse | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Shapes.Ellipse is not a collection
   # So if we're in a pipeline, make a new Ellipse each time
   if($_) { 
      $DObject = New-Object System.Windows.Shapes.Ellipse
   }
...
New-System.Windows.Shapes.Ellipse @PSBoundParameters
