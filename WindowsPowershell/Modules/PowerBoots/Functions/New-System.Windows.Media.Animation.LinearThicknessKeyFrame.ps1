...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "LinearThicknessKeyFrame not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "LinearThicknessKeyFrame in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Animation.LinearThicknessKeyFrame {
<#
.Synopsis
   Create a new LinearThicknessKeyFrame object
.Description
   Generates a new System.Windows.Media.Animation.LinearThicknessKeyFrame object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Animation.LinearThicknessKeyFrame
   $All = Get-Parameter New-System.Windows.Media.Animation.LinearThicknessKeyFrame | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Animation.LinearThicknessKeyFrame is not a collection
   # So if we're in a pipeline, make a new LinearThicknessKeyFrame each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Animation.LinearThicknessKeyFrame
   }
...
New-System.Windows.Media.Animation.LinearThicknessKeyFrame @PSBoundParameters
