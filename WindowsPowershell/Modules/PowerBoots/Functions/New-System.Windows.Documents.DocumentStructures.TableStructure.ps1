...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TableStructure not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TableStructure in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentStructures.TableStructure {
<#
.Synopsis
   Create a new TableStructure object
.Description
   Generates a new System.Windows.Documents.DocumentStructures.TableStructure object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentStructures.TableStructure
   $All = Get-Parameter New-System.Windows.Documents.DocumentStructures.TableStructure | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentStructures.TableStructure is not a collection
   # So if we're in a pipeline, make a new TableStructure each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentStructures.TableStructure
   }
...
New-System.Windows.Documents.DocumentStructures.TableStructure @PSBoundParameters
