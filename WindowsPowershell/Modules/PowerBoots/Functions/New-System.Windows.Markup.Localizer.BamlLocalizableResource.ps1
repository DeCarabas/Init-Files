...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "BamlLocalizableResource not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "BamlLocalizableResource in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.Localizer.BamlLocalizableResource {
<#
.Synopsis
   Create a new BamlLocalizableResource object
.Description
   Generates a new System.Windows.Markup.Localizer.BamlLocalizableResource object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.Localizer.BamlLocalizableResource
   $All = Get-Parameter New-System.Windows.Markup.Localizer.BamlLocalizableResource | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.Localizer.BamlLocalizableResource is not a collection
   # So if we're in a pipeline, make a new BamlLocalizableResource each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.Localizer.BamlLocalizableResource
   }
...
New-System.Windows.Markup.Localizer.BamlLocalizableResource @PSBoundParameters
