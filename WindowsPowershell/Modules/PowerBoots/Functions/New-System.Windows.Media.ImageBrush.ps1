...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ImageBrush not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ImageBrush in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.ImageBrush {
<#
.Synopsis
   Create a new ImageBrush object
.Description
   Generates a new System.Windows.Media.ImageBrush object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.ImageBrush
   $All = Get-Parameter New-System.Windows.Media.ImageBrush | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.ImageBrush is not a collection
   # So if we're in a pipeline, make a new ImageBrush each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.ImageBrush
   }
...
New-System.Windows.Media.ImageBrush @PSBoundParameters
