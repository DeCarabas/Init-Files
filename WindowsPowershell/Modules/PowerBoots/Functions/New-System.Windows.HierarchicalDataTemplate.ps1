...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "HierarchicalDataTemplate not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "HierarchicalDataTemplate in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.HierarchicalDataTemplate {
<#
.Synopsis
   Create a new HierarchicalDataTemplate object
.Description
   Generates a new System.Windows.HierarchicalDataTemplate object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.HierarchicalDataTemplate
   $All = Get-Parameter New-System.Windows.HierarchicalDataTemplate | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.HierarchicalDataTemplate is not a collection
   # So if we're in a pipeline, make a new HierarchicalDataTemplate each time
   if($_) { 
      $DObject = New-Object System.Windows.HierarchicalDataTemplate
   }
...
New-System.Windows.HierarchicalDataTemplate @PSBoundParameters
