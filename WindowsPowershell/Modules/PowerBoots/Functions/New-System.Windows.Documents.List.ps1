...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "List not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "List in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.List {
<#
.Synopsis
   Create a new List object
.Description
   Generates a new System.Windows.Documents.List object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.List
   $All = Get-Parameter New-System.Windows.Documents.List | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.List is not a collection
   # So if we're in a pipeline, make a new List each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.List
   }
...
New-System.Windows.Documents.List @PSBoundParameters
