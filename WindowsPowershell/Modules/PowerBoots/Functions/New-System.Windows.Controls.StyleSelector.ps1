...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "StyleSelector not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "StyleSelector in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.StyleSelector {
<#
.Synopsis
   Create a new StyleSelector object
.Description
   Generates a new System.Windows.Controls.StyleSelector object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.StyleSelector
   $All = Get-Parameter New-System.Windows.Controls.StyleSelector | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.StyleSelector is not a collection
   # So if we're in a pipeline, make a new StyleSelector each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.StyleSelector
   }
...
New-System.Windows.Controls.StyleSelector @PSBoundParameters
