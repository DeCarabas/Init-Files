...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "BindingGroup not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "BindingGroup in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Data.BindingGroup {
<#
.Synopsis
   Create a new BindingGroup object
.Description
   Generates a new System.Windows.Data.BindingGroup object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Data.BindingGroup
   $All = Get-Parameter New-System.Windows.Data.BindingGroup | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Data.BindingGroup is not a collection
   # So if we're in a pipeline, make a new BindingGroup each time
   if($_) { 
      $DObject = New-Object System.Windows.Data.BindingGroup
   }
...
New-System.Windows.Data.BindingGroup @PSBoundParameters
