...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "FlowDocumentPageViewer not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "FlowDocumentPageViewer in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.FlowDocumentPageViewer {
<#
.Synopsis
   Create a new FlowDocumentPageViewer object
.Description
   Generates a new System.Windows.Controls.FlowDocumentPageViewer object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.FlowDocumentPageViewer
   $All = Get-Parameter New-System.Windows.Controls.FlowDocumentPageViewer | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.FlowDocumentPageViewer is not a collection
   # So if we're in a pipeline, make a new FlowDocumentPageViewer each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.FlowDocumentPageViewer
   }
...
New-System.Windows.Controls.FlowDocumentPageViewer @PSBoundParameters
