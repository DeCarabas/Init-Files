...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "CommandConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "CommandConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Input.CommandConverter {
<#
.Synopsis
   Create a new CommandConverter object
.Description
   Generates a new System.Windows.Input.CommandConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Input.CommandConverter
   $All = Get-Parameter New-System.Windows.Input.CommandConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Input.CommandConverter is not a collection
   # So if we're in a pipeline, make a new CommandConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.Input.CommandConverter
   }
...
New-System.Windows.Input.CommandConverter @PSBoundParameters
