...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "XmlDataProvider not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "XmlDataProvider in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Data.XmlDataProvider {
<#
.Synopsis
   Create a new XmlDataProvider object
.Description
   Generates a new System.Windows.Data.XmlDataProvider object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Data.XmlDataProvider
   $All = Get-Parameter New-System.Windows.Data.XmlDataProvider | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Data.XmlDataProvider is not a collection
   # So if we're in a pipeline, make a new XmlDataProvider each time
   if($_) { 
      $DObject = New-Object System.Windows.Data.XmlDataProvider
   }
...
New-System.Windows.Data.XmlDataProvider @PSBoundParameters
