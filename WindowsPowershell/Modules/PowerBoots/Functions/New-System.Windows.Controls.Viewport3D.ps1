...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Viewport3D not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Viewport3D in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Viewport3D {
<#
.Synopsis
   Create a new Viewport3D object
.Description
   Generates a new System.Windows.Controls.Viewport3D object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Viewport3D
   $All = Get-Parameter New-System.Windows.Controls.Viewport3D | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Controls.Viewport3D @PSBoundParameters
