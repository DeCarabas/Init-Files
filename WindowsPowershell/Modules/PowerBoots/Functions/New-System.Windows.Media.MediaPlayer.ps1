...
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_MediaFailed
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_MediaOpened
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_MediaEnded
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_BufferingStarted
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_BufferingEnded
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_ScriptCommand
,
	[Parameter(ParameterSetName='Default')]
	[PSObject]$On_Changed
,
...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "MediaPlayer not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "MediaPlayer in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.MediaPlayer {
<#
.Synopsis
   Create a new MediaPlayer object
.Description
   Generates a new System.Windows.Media.MediaPlayer object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.MediaPlayer
   $All = Get-Parameter New-System.Windows.Media.MediaPlayer | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.MediaPlayer is not a collection
   # So if we're in a pipeline, make a new MediaPlayer each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.MediaPlayer
   }
...
New-System.Windows.Media.MediaPlayer @PSBoundParameters
