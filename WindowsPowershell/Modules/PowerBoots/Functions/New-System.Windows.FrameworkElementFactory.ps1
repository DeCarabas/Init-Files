...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "FrameworkElementFactory not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "FrameworkElementFactory in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.FrameworkElementFactory {
<#
.Synopsis
   Create a new FrameworkElementFactory object
.Description
   Generates a new System.Windows.FrameworkElementFactory object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.FrameworkElementFactory
   $All = Get-Parameter New-System.Windows.FrameworkElementFactory | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.FrameworkElementFactory is not a collection
   # So if we're in a pipeline, make a new FrameworkElementFactory each time
   if($_) { 
      $DObject = New-Object System.Windows.FrameworkElementFactory
   }
...
New-System.Windows.FrameworkElementFactory @PSBoundParameters
