...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ListBoxItem not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ListBoxItem in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.ListBoxItem {
<#
.Synopsis
   Create a new ListBoxItem object
.Description
   Generates a new System.Windows.Controls.ListBoxItem object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.ListBoxItem
   $All = Get-Parameter New-System.Windows.Controls.ListBoxItem | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.ListBoxItem is not a collection
   # So if we're in a pipeline, make a new ListBoxItem each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.ListBoxItem
   }
...
New-System.Windows.Controls.ListBoxItem @PSBoundParameters
