...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Image not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Image in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Image {
<#
.Synopsis
   Create a new Image object
.Description
   Generates a new System.Windows.Controls.Image object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Image
   $All = Get-Parameter New-System.Windows.Controls.Image | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Image is not a collection
   # So if we're in a pipeline, make a new Image each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Image
   }
...
New-System.Windows.Controls.Image @PSBoundParameters
