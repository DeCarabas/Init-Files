...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ImageDrawing not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ImageDrawing in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.ImageDrawing {
<#
.Synopsis
   Create a new ImageDrawing object
.Description
   Generates a new System.Windows.Media.ImageDrawing object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.ImageDrawing
   $All = Get-Parameter New-System.Windows.Media.ImageDrawing | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.ImageDrawing is not a collection
   # So if we're in a pipeline, make a new ImageDrawing each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.ImageDrawing
   }
...
New-System.Windows.Media.ImageDrawing @PSBoundParameters
