...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TemplateBindingExpressionConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TemplateBindingExpressionConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.TemplateBindingExpressionConverter {
<#
.Synopsis
   Create a new TemplateBindingExpressionConverter object
.Description
   Generates a new System.Windows.TemplateBindingExpressionConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.TemplateBindingExpressionConverter
   $All = Get-Parameter New-System.Windows.TemplateBindingExpressionConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.TemplateBindingExpressionConverter is not a collection
   # So if we're in a pipeline, make a new TemplateBindingExpressionConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.TemplateBindingExpressionConverter
   }
...
New-System.Windows.TemplateBindingExpressionConverter @PSBoundParameters
