...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TranslateTransform not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TranslateTransform in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.TranslateTransform {
<#
.Synopsis
   Create a new TranslateTransform object
.Description
   Generates a new System.Windows.Media.TranslateTransform object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.TranslateTransform
   $All = Get-Parameter New-System.Windows.Media.TranslateTransform | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.TranslateTransform is not a collection
   # So if we're in a pipeline, make a new TranslateTransform each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.TranslateTransform
   }
...
New-System.Windows.Media.TranslateTransform @PSBoundParameters
