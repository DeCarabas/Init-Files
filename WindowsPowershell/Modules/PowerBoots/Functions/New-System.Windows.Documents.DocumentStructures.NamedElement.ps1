...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "NamedElement not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "NamedElement in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentStructures.NamedElement {
<#
.Synopsis
   Create a new NamedElement object
.Description
   Generates a new System.Windows.Documents.DocumentStructures.NamedElement object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentStructures.NamedElement
   $All = Get-Parameter New-System.Windows.Documents.DocumentStructures.NamedElement | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentStructures.NamedElement is not a collection
   # So if we're in a pipeline, make a new NamedElement each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentStructures.NamedElement
   }
...
New-System.Windows.Documents.DocumentStructures.NamedElement @PSBoundParameters
