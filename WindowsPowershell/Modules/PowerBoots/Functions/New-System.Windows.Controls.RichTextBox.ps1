...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "RichTextBox not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "RichTextBox in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.RichTextBox {
<#
.Synopsis
   Create a new RichTextBox object
.Description
   Generates a new System.Windows.Controls.RichTextBox object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.RichTextBox
   $All = Get-Parameter New-System.Windows.Controls.RichTextBox | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.RichTextBox is not a collection
   # So if we're in a pipeline, make a new RichTextBox each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.RichTextBox
   }
...
New-System.Windows.Controls.RichTextBox @PSBoundParameters
