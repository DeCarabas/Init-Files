...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Polyline not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Polyline in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Shapes.Polyline {
<#
.Synopsis
   Create a new Polyline object
.Description
   Generates a new System.Windows.Shapes.Polyline object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Shapes.Polyline
   $All = Get-Parameter New-System.Windows.Shapes.Polyline | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Shapes.Polyline is not a collection
   # So if we're in a pipeline, make a new Polyline each time
   if($_) { 
      $DObject = New-Object System.Windows.Shapes.Polyline
   }
...
New-System.Windows.Shapes.Polyline @PSBoundParameters
