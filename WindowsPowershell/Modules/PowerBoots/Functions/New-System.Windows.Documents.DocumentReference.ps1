...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "DocumentReference not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "DocumentReference in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentReference {
<#
.Synopsis
   Create a new DocumentReference object
.Description
   Generates a new System.Windows.Documents.DocumentReference object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentReference
   $All = Get-Parameter New-System.Windows.Documents.DocumentReference | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentReference is not a collection
   # So if we're in a pipeline, make a new DocumentReference each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentReference
   }
...
New-System.Windows.Documents.DocumentReference @PSBoundParameters
