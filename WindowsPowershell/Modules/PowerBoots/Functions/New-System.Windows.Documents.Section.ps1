...
,
	[Parameter(ParameterSetName='Default')]
	[Switch]$HasTrailingParagraphBreakOnPaste
,
	[Parameter(ParameterSetName='Default')]
...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Section not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Section in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.Section {
<#
.Synopsis
   Create a new Section object
.Description
   Generates a new System.Windows.Documents.Section object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.Section
   $All = Get-Parameter New-System.Windows.Documents.Section | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Documents.Section @PSBoundParameters
