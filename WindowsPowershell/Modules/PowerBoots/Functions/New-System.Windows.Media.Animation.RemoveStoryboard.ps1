...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "RemoveStoryboard not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "RemoveStoryboard in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Animation.RemoveStoryboard {
<#
.Synopsis
   Create a new RemoveStoryboard object
.Description
   Generates a new System.Windows.Media.Animation.RemoveStoryboard object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Animation.RemoveStoryboard
   $All = Get-Parameter New-System.Windows.Media.Animation.RemoveStoryboard | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Animation.RemoveStoryboard is not a collection
   # So if we're in a pipeline, make a new RemoveStoryboard each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Animation.RemoveStoryboard
   }
...
New-System.Windows.Media.Animation.RemoveStoryboard @PSBoundParameters
