...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ThicknessAnimation not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ThicknessAnimation in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Animation.ThicknessAnimation {
<#
.Synopsis
   Create a new ThicknessAnimation object
.Description
   Generates a new System.Windows.Media.Animation.ThicknessAnimation object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Animation.ThicknessAnimation
   $All = Get-Parameter New-System.Windows.Media.Animation.ThicknessAnimation | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Animation.ThicknessAnimation is not a collection
   # So if we're in a pipeline, make a new ThicknessAnimation each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Animation.ThicknessAnimation
   }
...
New-System.Windows.Media.Animation.ThicknessAnimation @PSBoundParameters
