...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ToolBarTray not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ToolBarTray in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.ToolBarTray {
<#
.Synopsis
   Create a new ToolBarTray object
.Description
   Generates a new System.Windows.Controls.ToolBarTray object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.ToolBarTray
   $All = Get-Parameter New-System.Windows.Controls.ToolBarTray | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.ToolBarTray is not a collection
   # So if we're in a pipeline, make a new ToolBarTray each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.ToolBarTray
   }
...
New-System.Windows.Controls.ToolBarTray @PSBoundParameters
