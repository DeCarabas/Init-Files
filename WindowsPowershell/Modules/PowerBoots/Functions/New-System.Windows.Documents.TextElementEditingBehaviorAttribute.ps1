...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TextElementEditingBehaviorAttribute not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TextElementEditingBehaviorAttribute in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.TextElementEditingBehaviorAttribute {
<#
.Synopsis
   Create a new TextElementEditingBehaviorAttribute object
.Description
   Generates a new System.Windows.Documents.TextElementEditingBehaviorAttribute object, and allows setting all of it's properties
.Notes
 AUTHOR:    Joel Bennett http://HuddledMasses.org
 LASTEDIT:  07/23/2009 09:24:58
#>
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.TextElementEditingBehaviorAttribute
   $All = Get-Parameter New-System.Windows.Documents.TextElementEditingBehaviorAttribute | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.TextElementEditingBehaviorAttribute is not a collection
   # So if we're in a pipeline, make a new TextElementEditingBehaviorAttribute each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.TextElementEditingBehaviorAttribute
   }
...
New-System.Windows.Documents.TextElementEditingBehaviorAttribute @PSBoundParameters
