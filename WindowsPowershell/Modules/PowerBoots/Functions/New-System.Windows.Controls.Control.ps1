...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Control not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Control in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Control {
<#
.Synopsis
   Create a new Control object
.Description
   Generates a new System.Windows.Controls.Control object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Control
   $All = Get-Parameter New-System.Windows.Controls.Control | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Control is not a collection
   # So if we're in a pipeline, make a new Control each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Control
   }
...
New-System.Windows.Controls.Control @PSBoundParameters
