...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "StoryFragments not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "StoryFragments in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentStructures.StoryFragments {
<#
.Synopsis
   Create a new StoryFragments object
.Description
   Generates a new System.Windows.Documents.DocumentStructures.StoryFragments object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentStructures.StoryFragments
   $All = Get-Parameter New-System.Windows.Documents.DocumentStructures.StoryFragments | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentStructures.StoryFragments is not a collection
   # So if we're in a pipeline, make a new StoryFragments each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentStructures.StoryFragments
   }
...
New-System.Windows.Documents.DocumentStructures.StoryFragments @PSBoundParameters
