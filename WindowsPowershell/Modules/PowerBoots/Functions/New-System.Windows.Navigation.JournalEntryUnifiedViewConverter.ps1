...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "JournalEntryUnifiedViewConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "JournalEntryUnifiedViewConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Navigation.JournalEntryUnifiedViewConverter {
<#
.Synopsis
   Create a new JournalEntryUnifiedViewConverter object
.Description
   Generates a new System.Windows.Navigation.JournalEntryUnifiedViewConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Navigation.JournalEntryUnifiedViewConverter
   $All = Get-Parameter New-System.Windows.Navigation.JournalEntryUnifiedViewConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Navigation.JournalEntryUnifiedViewConverter is not a collection
   # So if we're in a pipeline, make a new JournalEntryUnifiedViewConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.Navigation.JournalEntryUnifiedViewConverter
   }
...
New-System.Windows.Navigation.JournalEntryUnifiedViewConverter @PSBoundParameters
