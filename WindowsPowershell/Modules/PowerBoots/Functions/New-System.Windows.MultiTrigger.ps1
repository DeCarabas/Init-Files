...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "MultiTrigger not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "MultiTrigger in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.MultiTrigger {
<#
.Synopsis
   Create a new MultiTrigger object
.Description
   Generates a new System.Windows.MultiTrigger object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.MultiTrigger
   $All = Get-Parameter New-System.Windows.MultiTrigger | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.MultiTrigger is not a collection
   # So if we're in a pipeline, make a new MultiTrigger each time
   if($_) { 
      $DObject = New-Object System.Windows.MultiTrigger
   }
...
New-System.Windows.MultiTrigger @PSBoundParameters
