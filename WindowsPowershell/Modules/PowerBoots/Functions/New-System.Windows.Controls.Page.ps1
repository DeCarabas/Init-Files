...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Page not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Page in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Page {
<#
.Synopsis
   Create a new Page object
.Description
   Generates a new System.Windows.Controls.Page object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Page
   $All = Get-Parameter New-System.Windows.Controls.Page | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Page is not a collection
   # So if we're in a pipeline, make a new Page each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Page
   }
...
New-System.Windows.Controls.Page @PSBoundParameters
