...
,
	[Parameter(ParameterSetName='Default')]
	[Switch]$IsFindEnabled
,
	[Parameter(ParameterSetName='Default')]
	[Switch]$IsHitTestVisible
,
	[Parameter(ParameterSetName='Default')]
	[Switch]$IsPageViewEnabled
,
	[Parameter(ParameterSetName='Default')]
	[Switch]$IsPrintEnabled
,
	[Parameter(ParameterSetName='Default')]
	[Switch]$IsScrollViewEnabled
,
	[Parameter(ParameterSetName='Default')]
	[Switch]$IsTabStop
,
	[Parameter(ParameterSetName='Default')]
	[Switch]$IsTwoPageViewEnabled
,
	[Parameter(ParameterSetName='Default')]
	[Object[]]$Language
,
...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "FlowDocumentReader not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "FlowDocumentReader in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.FlowDocumentReader {
<#
.Synopsis
   Create a new FlowDocumentReader object
.Description
   Generates a new System.Windows.Controls.FlowDocumentReader object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.FlowDocumentReader
   $All = Get-Parameter New-System.Windows.Controls.FlowDocumentReader | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.FlowDocumentReader is not a collection
   # So if we're in a pipeline, make a new FlowDocumentReader each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.FlowDocumentReader
   }
...
New-System.Windows.Controls.FlowDocumentReader @PSBoundParameters
