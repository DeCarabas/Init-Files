...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "FrameworkElement not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "FrameworkElement in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.FrameworkElement {
<#
.Synopsis
   Create a new FrameworkElement object
.Description
   Generates a new System.Windows.FrameworkElement object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.FrameworkElement
   $All = Get-Parameter New-System.Windows.FrameworkElement | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.FrameworkElement is not a collection
   # So if we're in a pipeline, make a new FrameworkElement each time
   if($_) { 
      $DObject = New-Object System.Windows.FrameworkElement
   }
...
New-System.Windows.FrameworkElement @PSBoundParameters
