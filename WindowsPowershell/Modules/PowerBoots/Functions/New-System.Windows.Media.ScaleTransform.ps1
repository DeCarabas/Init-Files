...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ScaleTransform not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ScaleTransform in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.ScaleTransform {
<#
.Synopsis
   Create a new ScaleTransform object
.Description
   Generates a new System.Windows.Media.ScaleTransform object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.ScaleTransform
   $All = Get-Parameter New-System.Windows.Media.ScaleTransform | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.ScaleTransform is not a collection
   # So if we're in a pipeline, make a new ScaleTransform each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.ScaleTransform
   }
...
New-System.Windows.Media.ScaleTransform @PSBoundParameters
