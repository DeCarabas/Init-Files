...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ComponentResourceKeyConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ComponentResourceKeyConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.ComponentResourceKeyConverter {
<#
.Synopsis
   Create a new ComponentResourceKeyConverter object
.Description
   Generates a new System.Windows.Markup.ComponentResourceKeyConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.ComponentResourceKeyConverter
   $All = Get-Parameter New-System.Windows.Markup.ComponentResourceKeyConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.ComponentResourceKeyConverter is not a collection
   # So if we're in a pipeline, make a new ComponentResourceKeyConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.ComponentResourceKeyConverter
   }
...
New-System.Windows.Markup.ComponentResourceKeyConverter @PSBoundParameters
