...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "DependencyPropertyConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "DependencyPropertyConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.DependencyPropertyConverter {
<#
.Synopsis
   Create a new DependencyPropertyConverter object
.Description
   Generates a new System.Windows.Markup.DependencyPropertyConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.DependencyPropertyConverter
   $All = Get-Parameter New-System.Windows.Markup.DependencyPropertyConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.DependencyPropertyConverter is not a collection
   # So if we're in a pipeline, make a new DependencyPropertyConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.DependencyPropertyConverter
   }
...
New-System.Windows.Markup.DependencyPropertyConverter @PSBoundParameters
