...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "HeaderedContentControl not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "HeaderedContentControl in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.HeaderedContentControl {
<#
.Synopsis
   Create a new HeaderedContentControl object
.Description
   Generates a new System.Windows.Controls.HeaderedContentControl object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.HeaderedContentControl
   $All = Get-Parameter New-System.Windows.Controls.HeaderedContentControl | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.HeaderedContentControl is not a collection
   # So if we're in a pipeline, make a new HeaderedContentControl each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.HeaderedContentControl
   }
...
New-System.Windows.Controls.HeaderedContentControl @PSBoundParameters
