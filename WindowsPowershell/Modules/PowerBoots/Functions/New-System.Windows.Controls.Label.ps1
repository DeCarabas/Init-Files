...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Label not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Label in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Label {
<#
.Synopsis
   Create a new Label object
.Description
   Generates a new System.Windows.Controls.Label object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Label
   $All = Get-Parameter New-System.Windows.Controls.Label | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Label is not a collection
   # So if we're in a pipeline, make a new Label each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Label
   }
...
New-System.Windows.Controls.Label @PSBoundParameters
