...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "LengthConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "LengthConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.LengthConverter {
<#
.Synopsis
   Create a new LengthConverter object
.Description
   Generates a new System.Windows.LengthConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.LengthConverter
   $All = Get-Parameter New-System.Windows.LengthConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.LengthConverter is not a collection
   # So if we're in a pipeline, make a new LengthConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.LengthConverter
   }
...
New-System.Windows.LengthConverter @PSBoundParameters
