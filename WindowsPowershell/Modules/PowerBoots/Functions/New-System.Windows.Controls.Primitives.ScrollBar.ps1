...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ScrollBar not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ScrollBar in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.Primitives.ScrollBar {
<#
.Synopsis
   Create a new ScrollBar object
.Description
   Generates a new System.Windows.Controls.Primitives.ScrollBar object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.Primitives.ScrollBar
   $All = Get-Parameter New-System.Windows.Controls.Primitives.ScrollBar | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.Primitives.ScrollBar is not a collection
   # So if we're in a pipeline, make a new ScrollBar each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.Primitives.ScrollBar
   }
...
New-System.Windows.Controls.Primitives.ScrollBar @PSBoundParameters
