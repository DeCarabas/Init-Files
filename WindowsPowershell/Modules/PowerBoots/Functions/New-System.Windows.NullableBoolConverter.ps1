...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "NullableBoolConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "NullableBoolConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.NullableBoolConverter {
<#
.Synopsis
   Create a new NullableBoolConverter object
.Description
   Generates a new System.Windows.NullableBoolConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.NullableBoolConverter
   $All = Get-Parameter New-System.Windows.NullableBoolConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.NullableBoolConverter is not a collection
   # So if we're in a pipeline, make a new NullableBoolConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.NullableBoolConverter
   }
...
New-System.Windows.NullableBoolConverter @PSBoundParameters
