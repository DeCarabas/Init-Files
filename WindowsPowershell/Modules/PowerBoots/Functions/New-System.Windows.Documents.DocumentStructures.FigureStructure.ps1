...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "FigureStructure not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "FigureStructure in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentStructures.FigureStructure {
<#
.Synopsis
   Create a new FigureStructure object
.Description
   Generates a new System.Windows.Documents.DocumentStructures.FigureStructure object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentStructures.FigureStructure
   $All = Get-Parameter New-System.Windows.Documents.DocumentStructures.FigureStructure | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentStructures.FigureStructure is not a collection
   # So if we're in a pipeline, make a new FigureStructure each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentStructures.FigureStructure
   }
...
New-System.Windows.Documents.DocumentStructures.FigureStructure @PSBoundParameters
