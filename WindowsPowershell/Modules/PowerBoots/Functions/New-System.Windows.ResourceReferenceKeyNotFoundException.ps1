...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ResourceReferenceKeyNotFoundException not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ResourceReferenceKeyNotFoundException in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.ResourceReferenceKeyNotFoundException {
<#
.Synopsis
   Create a new ResourceReferenceKeyNotFoundException object
.Description
   Generates a new System.Windows.ResourceReferenceKeyNotFoundException object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.ResourceReferenceKeyNotFoundException
   $All = Get-Parameter New-System.Windows.ResourceReferenceKeyNotFoundException | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.ResourceReferenceKeyNotFoundException is not a collection
   # So if we're in a pipeline, make a new ResourceReferenceKeyNotFoundException each time
   if($_) { 
      $DObject = New-Object System.Windows.ResourceReferenceKeyNotFoundException
   }
...
New-System.Windows.ResourceReferenceKeyNotFoundException @PSBoundParameters
