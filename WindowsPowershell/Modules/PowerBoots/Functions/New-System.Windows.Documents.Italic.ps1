...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "Italic not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "Italic in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.Italic {
<#
.Synopsis
   Create a new Italic object
.Description
   Generates a new System.Windows.Documents.Italic object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.Italic
   $All = Get-Parameter New-System.Windows.Documents.Italic | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Documents.Italic @PSBoundParameters
