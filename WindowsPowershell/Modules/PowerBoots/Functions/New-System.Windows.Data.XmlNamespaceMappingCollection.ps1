...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "XmlNamespaceMappingCollection not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "XmlNamespaceMappingCollection in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Data.XmlNamespaceMappingCollection {
<#
.Synopsis
   Create a new XmlNamespaceMappingCollection object
.Description
   Generates a new System.Windows.Data.XmlNamespaceMappingCollection object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Data.XmlNamespaceMappingCollection
   $All = Get-Parameter New-System.Windows.Data.XmlNamespaceMappingCollection | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Data.XmlNamespaceMappingCollection is not a collection
   # So if we're in a pipeline, make a new XmlNamespaceMappingCollection each time
   if($_) { 
      $DObject = New-Object System.Windows.Data.XmlNamespaceMappingCollection
   }
...
New-System.Windows.Data.XmlNamespaceMappingCollection @PSBoundParameters
