...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "XmlNamespaceMapping not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "XmlNamespaceMapping in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Data.XmlNamespaceMapping {
<#
.Synopsis
   Create a new XmlNamespaceMapping object
.Description
   Generates a new System.Windows.Data.XmlNamespaceMapping object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Data.XmlNamespaceMapping
   $All = Get-Parameter New-System.Windows.Data.XmlNamespaceMapping | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Data.XmlNamespaceMapping is not a collection
   # So if we're in a pipeline, make a new XmlNamespaceMapping each time
   if($_) { 
      $DObject = New-Object System.Windows.Data.XmlNamespaceMapping
   }
...
New-System.Windows.Data.XmlNamespaceMapping @PSBoundParameters
