...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "DataTrigger not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "DataTrigger in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.DataTrigger {
<#
.Synopsis
   Create a new DataTrigger object
.Description
   Generates a new System.Windows.DataTrigger object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.DataTrigger
   $All = Get-Parameter New-System.Windows.DataTrigger | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.DataTrigger is not a collection
   # So if we're in a pipeline, make a new DataTrigger each time
   if($_) { 
      $DObject = New-Object System.Windows.DataTrigger
   }
...
New-System.Windows.DataTrigger @PSBoundParameters
