...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TableRowGroup not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TableRowGroup in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.TableRowGroup {
<#
.Synopsis
   Create a new TableRowGroup object
.Description
   Generates a new System.Windows.Documents.TableRowGroup object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.TableRowGroup
   $All = Get-Parameter New-System.Windows.Documents.TableRowGroup | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.TableRowGroup is not a collection
   # So if we're in a pipeline, make a new TableRowGroup each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.TableRowGroup
   }
...
New-System.Windows.Documents.TableRowGroup @PSBoundParameters
