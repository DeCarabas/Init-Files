...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "FontSizeConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "FontSizeConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.FontSizeConverter {
<#
.Synopsis
   Create a new FontSizeConverter object
.Description
   Generates a new System.Windows.FontSizeConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.FontSizeConverter
   $All = Get-Parameter New-System.Windows.FontSizeConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.FontSizeConverter is not a collection
   # So if we're in a pipeline, make a new FontSizeConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.FontSizeConverter
   }
...
New-System.Windows.FontSizeConverter @PSBoundParameters
