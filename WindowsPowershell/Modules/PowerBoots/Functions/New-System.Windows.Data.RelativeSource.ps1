...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "RelativeSource not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "RelativeSource in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Data.RelativeSource {
<#
.Synopsis
   Create a new RelativeSource object
.Description
   Generates a new System.Windows.Data.RelativeSource object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Data.RelativeSource
   $All = Get-Parameter New-System.Windows.Data.RelativeSource | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Data.RelativeSource is not a collection
   # So if we're in a pipeline, make a new RelativeSource each time
   if($_) { 
      $DObject = New-Object System.Windows.Data.RelativeSource
   }
...
New-System.Windows.Data.RelativeSource @PSBoundParameters
