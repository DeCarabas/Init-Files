...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "DropShadowBitmapEffect not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "DropShadowBitmapEffect in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Effects.DropShadowBitmapEffect {
<#
.Synopsis
   Create a new DropShadowBitmapEffect object
.Description
   Generates a new System.Windows.Media.Effects.DropShadowBitmapEffect object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Effects.DropShadowBitmapEffect
   $All = Get-Parameter New-System.Windows.Media.Effects.DropShadowBitmapEffect | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Media.Effects.DropShadowBitmapEffect is not a collection
   # So if we're in a pipeline, make a new DropShadowBitmapEffect each time
   if($_) { 
      $DObject = New-Object System.Windows.Media.Effects.DropShadowBitmapEffect
   }
...
New-System.Windows.Media.Effects.DropShadowBitmapEffect @PSBoundParameters
