...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "LinkTarget not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "LinkTarget in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.LinkTarget {
<#
.Synopsis
   Create a new LinkTarget object
.Description
   Generates a new System.Windows.Documents.LinkTarget object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.LinkTarget
   $All = Get-Parameter New-System.Windows.Documents.LinkTarget | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.LinkTarget is not a collection
   # So if we're in a pipeline, make a new LinkTarget each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.LinkTarget
   }
...
New-System.Windows.Documents.LinkTarget @PSBoundParameters
