...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TemplateKeyConverter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TemplateKeyConverter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.TemplateKeyConverter {
<#
.Synopsis
   Create a new TemplateKeyConverter object
.Description
   Generates a new System.Windows.Markup.TemplateKeyConverter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.TemplateKeyConverter
   $All = Get-Parameter New-System.Windows.Markup.TemplateKeyConverter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.TemplateKeyConverter is not a collection
   # So if we're in a pipeline, make a new TemplateKeyConverter each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.TemplateKeyConverter
   }
...
New-System.Windows.Markup.TemplateKeyConverter @PSBoundParameters
