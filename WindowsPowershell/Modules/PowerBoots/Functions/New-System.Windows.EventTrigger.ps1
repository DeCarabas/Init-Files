...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "EventTrigger not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "EventTrigger in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.EventTrigger {
<#
.Synopsis
   Create a new EventTrigger object
.Description
   Generates a new System.Windows.EventTrigger object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.EventTrigger
   $All = Get-Parameter New-System.Windows.EventTrigger | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.EventTrigger is not a collection
   # So if we're in a pipeline, make a new EventTrigger each time
   if($_) { 
      $DObject = New-Object System.Windows.EventTrigger
   }
...
New-System.Windows.EventTrigger @PSBoundParameters
