...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "XamlReader not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "XamlReader in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Markup.XamlReader {
<#
.Synopsis
   Create a new XamlReader object
.Description
   Generates a new System.Windows.Markup.XamlReader object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Markup.XamlReader
   $All = Get-Parameter New-System.Windows.Markup.XamlReader | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Markup.XamlReader is not a collection
   # So if we're in a pipeline, make a new XamlReader each time
   if($_) { 
      $DObject = New-Object System.Windows.Markup.XamlReader
   }
...
New-System.Windows.Markup.XamlReader @PSBoundParameters
