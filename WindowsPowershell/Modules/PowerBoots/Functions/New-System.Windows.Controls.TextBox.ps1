...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "TextBox not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "TextBox in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.TextBox {
<#
.Synopsis
   Create a new TextBox object
.Description
   Generates a new System.Windows.Controls.TextBox object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.TextBox
   $All = Get-Parameter New-System.Windows.Controls.TextBox | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.TextBox is not a collection
   # So if we're in a pipeline, make a new TextBox each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.TextBox
   }
...
New-System.Windows.Controls.TextBox @PSBoundParameters
