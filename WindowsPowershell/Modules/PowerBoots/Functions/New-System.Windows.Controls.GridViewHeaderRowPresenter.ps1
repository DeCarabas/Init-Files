...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "GridViewHeaderRowPresenter not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "GridViewHeaderRowPresenter in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.GridViewHeaderRowPresenter {
<#
.Synopsis
   Create a new GridViewHeaderRowPresenter object
.Description
   Generates a new System.Windows.Controls.GridViewHeaderRowPresenter object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.GridViewHeaderRowPresenter
   $All = Get-Parameter New-System.Windows.Controls.GridViewHeaderRowPresenter | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.GridViewHeaderRowPresenter is not a collection
   # So if we're in a pipeline, make a new GridViewHeaderRowPresenter each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.GridViewHeaderRowPresenter
   }
...
New-System.Windows.Controls.GridViewHeaderRowPresenter @PSBoundParameters
