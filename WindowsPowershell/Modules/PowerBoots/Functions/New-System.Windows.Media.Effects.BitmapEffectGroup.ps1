...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "BitmapEffectGroup not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "BitmapEffectGroup in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Media.Effects.BitmapEffectGroup {
<#
.Synopsis
   Create a new BitmapEffectGroup object
.Description
   Generates a new System.Windows.Media.Effects.BitmapEffectGroup object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Media.Effects.BitmapEffectGroup
   $All = Get-Parameter New-System.Windows.Media.Effects.BitmapEffectGroup | ForEach-Object { $_.Key } | Sort
}
PROCESS {
...
New-System.Windows.Media.Effects.BitmapEffectGroup @PSBoundParameters
