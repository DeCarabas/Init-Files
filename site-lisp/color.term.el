...
(defvar term-terminal-previous-parameters nil)
...
(defun term--color-face (color)
  "A helper to create a face spec with foreground and background COLOR."
  (list (list t :foreground color :background color)))

(defface term-color-black
  (term--color-face "#192033")
  "Face used to render black color code."
  :group 'term)

(defface term-color-red
  (term--color-face "#A93F43")
  "Face used to render red color code."
  :group 'term)

(defface term-color-green
  (term--color-face "#59963A")
  "Face used to render green color code."
  :group 'term)

(defface term-color-yellow
  (term--color-face "#BE8A2D")
  "Face used to render yellow color code."
  :group 'term)

(defface term-color-blue
  (term--color-face "#4068A3")
  "Face used to render blue color code."
  :group 'term)

(defface term-color-magenta
  (term--color-face "#7F60A7")
  "Face used to render magenta color code."
  :group 'term)

(defface term-color-cyan
  (term--color-face "#4E9B9B")
  "Face used to render cyan color code."
  :group 'term)

(defface term-color-white
  (term--color-face "#7E8A90")
  "Face used to render white color code."
  :group 'term)

(defface term-color-bright-black
  (term--color-face "#666666")
  "Face used to render black color code."
  :group 'term)

(defface term-color-bright-red
  (term--color-face "#EC6261")
  "Face used to render red color code."
  :group 'term)

(defface term-color-bright-green
  (term--color-face "#ADCF44")
  "Face used to render green color code."
  :group 'term)

(defface term-color-bright-yellow
  (term--color-face "#F0C649")
  "Face used to render yellow color code."
  :group 'term)
...
(defface term-color-bright-magenta
  (term--color-face "#CB77F9")
  "Face used to render magenta color code."
  :group 'term)

(defface term-color-bright-cyan
  (term--color-face "#86D7DB")
  "Face used to render cyan color code."
  :group 'term)

(defface term-color-bright-white
  (term--color-face "#D3D2D1")
  "Face used to render white color code."
  :group 'term)
...
(defcustom term-debug nil
  "Print term debuging information in *Messages* if T."
  :type 'boolean
  :group 'term)

(defun term--message (format-string &rest args)
...
Entry to this mode runs the hooks on `term-mode-hook'."
  ...
  (make-local-variable 'term-terminal-previous-parameters)
...
PROC is the process being filtered, and STR is the string it
generated.  It emulates (most of the features of) a
VT100/ANSI-style terminal."
        ...
        (save-restriction
          ...
          (while (< i str-length)
            ...
            (cond
             ((< term-terminal-state 2)
              ...
              (cond
               ((> funny i)
                ...
                (cond
                 ((eq term-terminal-state 1)
                  ;; We are in state 1, we need to wrap around.  Go to the
                  ;; beginning of the next line and switch to state 0.
                  (term-down 1 t)
                ...
                (cond
                 ...
                 ((or term-suppress-hard-newline (<= temp 0)))

                 ((> count temp)
                  ;; Some chars fit. This iteration, handle only what fits.
                ...
                ;; Insert a string, check how many columns we moved, then
                ;; delete that many columns following point if not eob nor
                ;; insert-mode.
                (let ((old-column (current-column))
                  ...
                  ;; In insert mode if the current line has become too long
                  ;; it needs to be chopped off.
                ...
                ;; If the last char was written in last column, back up one
                ;; column, but remember we did so.  Thus we emulate
                ;; xterm/vt100-style line-wrapping.
                (when (eq temp 0)
                  (term-move-columns -1)
             ...
             ((eq term-terminal-state 2)     ; Seen Esc
              (cond
               ((eq char ?\[)     ;; ?\133 = ?[
                ...
                (setq term-terminal-previous-parameters nil)
                (setq term-terminal-state 3))

               ((eq char ?\])
                (setq term-terminal-parameter 0)
                (setq term-terminal-previous-parameters nil)
                (setq term-terminal-state 5))
               ...
               ((eq char ?8) ;; Restore cursor (terminfo: rc)
                (when term-saved-cursor
                  (term-goto (nth 0 term-saved-cursor) (nth 1 term-saved-cursor))
                  (setq term-ansi-current-bg-color (nth 2 term-saved-cursor)
                        term-ansi-current-bold (nth 3 term-saved-cursor)
                        term-ansi-current-color (nth 4 term-saved-cursor)
                        term-ansi-current-invisible (nth 5 term-saved-cursor)
                        term-ansi-current-reverse (nth 6 term-saved-cursor)
                        term-ansi-current-underline (nth 7 term-saved-cursor)
                        term-current-face (nth 8 term-saved-cursor)))
                (setq term-terminal-state 0))
             ...
             ((eq term-terminal-state 3) ; Seen Esc [ (CSI)
              (cond
               ((and (>= char ?0) (<= char ?9))
               ...
               ((eq char ?\;)
                (push term-terminal-parameter term-terminal-previous-parameters)
                (setq term-terminal-parameter 0))
             ...
             ;; OSC (ESC ] ...)
             ;; This is terminated by either BEL or ESC \, which means we need
             ;; another state to deal with escapes...
             ((eq term-terminal-state 5) ; Seen Esc ] (OSC)
              (cond
               ((eq char ?\;)
                (push term-terminal-parameter term-terminal-previous-parameters)
                (setq term-terminal-parameter 0))
               ...
               ((eq char 7)
                (push term-terminal-parameter term-terminal-previous-parameters)
                (setq term-terminal-parameter 0)
                (term-handle-osc proc)
                (setq term-terminal-previous-parameters nil)
                (setq term-terminal-state 0))

               ((eq char ?\033) ; Escape
                (setq term-terminal-state 6))

               ;; TODO: Capture input
               (t)))

             ((eq term-terminal-state 6) ; Seen Esc ] ... Esc
              (cond
               ((eq char ?\\) ; ST
                (push term-terminal-parameter term-terminal-previous-parameters)
                (setq term-terminal-parameter 0)
...
(defun term--grayscale-colorp (color)
  "Return t if COLOR is a grayscale color index."
  (and (>= color 232)
       (<= color 255)))

(defun term--grayscale-color-value (color)
  "Decode the incoming COLOR into the appropriate greyscale color.

COLOR must be between 232 and 255 to make sense."
  (let ((val (+ 8 (* (- color 232) 10))))
      (format "#%02x%02x%02x"
              val val val)))

(defun term--256-color-value (color)
  "Decode the incoming COLOR into an appopriate RGB color string."
  (let* ((color-table [0 #x5f #x87 #xaf #xd7 #xff])
         (color (- color 16))
         (red (/ color 36))
         (color (mod color 36))
         (green (/ color 6))
...
(defun term--ansi-colorp (color)
  "Return t if COLOR is an ANSI color index."
  (or (not color)
      (and (>= color 0)
           (<= color 15))))

(defun term--ansi-color-value (color face-func)
  "Decode the incoming COLOR as an ANSI color with FACE-FUNC.

If color is nil, then this returns the default color.  Otherwise it should be
between 0 and 15, described by ansi-term-color-vector."
  (let* ((index (if (not color) 0 (1+ color)))
         (face (elt ansi-term-color-vector index)))
    (funcall face-func face nil 'default)))

(defun term--color-value (color face-func)
  "I don't know COLOR and FACE-FUNC."
  (cond
   ((term--ansi-colorp color)
    (term--ansi-color-value color face-func))

   ((term--grayscale-colorp color)
    (term--grayscale-color-value color))

   (t
    (term--256-color-value color))))

(defun term--fg-color-value (value)
  "Decode the incoming VALUE into the appropriate foreground color string."
  (term--color-value value 'face-foreground))

(defun term--bg-color-value (value)
  "Decode the incoming VALUE into the appropriate background color string."
  (term--color-value value 'face-background))

(defun term--resolve-color (face-func bright color)
  "Use FACE-FUNC to resolve the (possibly BRIGHT) COLOR.

FACE-FUNC is something like 'face-background' or 'face-foreground'.  bold is t
if the color is supposed to be bright, otherwise nil for regular.  color can
either be a number between 0 and 7 which will be a stock ANSI color, otherwise
it will be a string representing an actual color name."
  (if (stringp color)
      color
    (let* ((bright-plus (if bright 8 0))
           (index (if (not color) 0 (+ 1 bright-plus color)))
           (face (elt ansi-term-color-vector index)))
      (funcall face-func face nil 'default))))

(defun term--resolve-fg (bright color)
  "Resolve the (possibly BRIGHT) COLOR to a foreground color name."
  (term--resolve-color 'face-foreground bright color))

(defun term--resolve-bg (bright color)
  "Resolve the (possibly BRIGHT) COLOR to a background color name."
  (term--resolve-color 'face-background bright color))


(defun term--process-sgr-command (parameters)
  "Process first SGR command in PARAMETERS, returning the rest of the list.

PARAMETERS is a list of integers.  As a side effect, this function updates the
various term-ansi-current-* variables."
  (let ((parameter (first parameters)))
    (cond
     ...
     ((and (>= parameter 30) (<= parameter 37))
      (setq term-ansi-current-color (- parameter 30))
      (cdr parameters))

     ;; XTERM 256 FG color
     ((eq parameter 38)
      (setq term-ansi-current-color
            (term--fg-color-value (nth 2 parameters)))
      (cdddr parameters))
     ...
     ((and (>= parameter 40) (<= parameter 47))
      (setq term-ansi-current-bg-color (- parameter 40))
      (cdr parameters))

     ;; XTERM 256 BG color
     ((eq parameter 48)
      (setq term-ansi-current-bg-color
            (term--bg-color-value (nth 2 parameters)))
      (cdddr parameters))
...
(defun term-handle-colors-array (parameters)
  "Handle the SGR commands in PARAMETERS."
  (while parameters
    (setq parameters (term--process-sgr-command parameters)))

  (term--message "Debug: U-%s R-%s B-%s I-%s D-%s F-%s B-%s"
                 term-ansi-current-underline
                 term-ansi-current-reverse
                 term-ansi-current-bold
                 term-ansi-current-invisible
    ...
    (if term-ansi-current-invisible
        (let ((color
          ...
          (setq term-current-face
                (list
                 :background (term--resolve-fg term-ansi-current-bold color)
                 :foreground (term--resolve-bg term-ansi-current-bold color)))
  ...
  (term--message "Debug %S" term-current-face)
...
That is, we've previously seen Escape followed by ?[.  PROC is the process
we're filtering for, CHAR is the char we've seen."
  (setq term-terminal-previous-parameters
        (nreverse term-terminal-previous-parameters))
  (setq term-terminal-parameter (first term-terminal-previous-parameters))
  ...
  (cond
   ((or (eq char ?H)  ;; cursor motion (terminfo: cup,home)
    ...
    (let ((row (nth 0 term-terminal-previous-parameters))
          (col (nth 1 term-terminal-previous-parameters)))
      (if (not row) (setq row 1))
      (if (not col) (setq col 1))
      (when (<= row 0) (setq row 1))
      (when (<= col 0) (setq col 1))
      (when (> row term-height) (setq row term-height))
      (when (> col term-width) (setq col term-width))
      (term-goto (1- row) (1- col))))
   ...
   ((eq char ?m)
    (term-handle-colors-array term-terminal-previous-parameters))

   (t)))

(defun term-handle-osc (proc)
  "Handle an OSC message for PROC, assuming we're in state 5 or 6.

TODO: Implement this, it looks useful!"
  nil)
