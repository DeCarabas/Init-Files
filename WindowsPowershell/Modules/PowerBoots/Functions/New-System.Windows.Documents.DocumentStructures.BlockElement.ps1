...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "BlockElement not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "BlockElement in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Documents.DocumentStructures.BlockElement {
<#
.Synopsis
   Create a new BlockElement object
.Description
   Generates a new System.Windows.Documents.DocumentStructures.BlockElement object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Documents.DocumentStructures.BlockElement
   $All = Get-Parameter New-System.Windows.Documents.DocumentStructures.BlockElement | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Documents.DocumentStructures.BlockElement is not a collection
   # So if we're in a pipeline, make a new BlockElement each time
   if($_) { 
      $DObject = New-Object System.Windows.Documents.DocumentStructures.BlockElement
   }
...
New-System.Windows.Documents.DocumentStructures.BlockElement @PSBoundParameters
