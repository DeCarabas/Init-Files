...
if($ExecutionContext.SessionState.Module.Guid -ne (Get-BootsModule).Guid) {
	Write-Warning "ContentControl not invoked in PowerBoots context. Attempting to reinvoke."
   $scriptParam = $PSBoundParameters
   return iex "& (Get-BootsModule) '$($MyInvocation.MyCommand.Path)' `@PSBoundParameters"
}
# Write-Host "ContentControl in module $($executioncontext.sessionstate.module) context!" -fore Green


function Global:New-System.Windows.Controls.ContentControl {
<#
.Synopsis
   Create a new ContentControl object
.Description
   Generates a new System.Windows.Controls.ContentControl object, and allows setting all of it's properties
.Notes
...
BEGIN {
   $DObject = New-Object System.Windows.Controls.ContentControl
   $All = Get-Parameter New-System.Windows.Controls.ContentControl | ForEach-Object { $_.Key } | Sort
}
PROCESS {


   # The content of System.Windows.Controls.ContentControl is not a collection
   # So if we're in a pipeline, make a new ContentControl each time
   if($_) { 
      $DObject = New-Object System.Windows.Controls.ContentControl
   }
...
New-System.Windows.Controls.ContentControl @PSBoundParameters
